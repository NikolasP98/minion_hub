...
| drop        | `src/lib/components/dashboard/date-range/date-range.test.ts`              | Inclusive-bound behavior exists on current master; keep current tests.                      |
| drop        | `src/lib/components/dashboard/date-range/index.ts`                        | Current export is authoritative; old head previously carried duplicate-export breakage.     |
| drop        | `src/lib/components/dashboard/date-range/periods.ts`                      | Inclusive range behavior is already represented in the current date-range implementation.   |
