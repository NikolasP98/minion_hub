...
Choose the starter template you want to explore.
...
Let's install electrobun. Just run:
...
> **Note:** We've modified the default package.json that bun creates:
...
One last thing, we need a way to let the Electrobun cli know where our bun entrypoint file is and how to build it. We do that by creating an `electrobun.config.ts` file in the root of the project.
