  ...
  onMount(() => {
    ...
    const stopLoop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        ...
        ctx.fillRect(updates % 250, 30, 20, 20);
      },
    });
    const stop = () => {
      stopped = true;
    ...
    Object.assign(window, {
      __workshop: {
        snapshot: () => ({
          ...
          liveSeen,
          stopped,
          characters: office.getCharacters().map((ch) => ({
            id: ch.id,
            effect: ch.matrixEffect,
...
{#if task}
  <TaskPromptDialog
    mode="assign"
    agentName={task === 'agent-a' ? 'Ada' : 'Grace'}
    {value}
    onValueChange={(next) => (value = next)}
    onCancel={() => (task = null)}
    onSubmit={() => {
      submitted = `${task}: ${value}`;
      task = null;
    }}
  />
{/if}
