...
--   directly (recorded in operator memory
--   `sdlc-board-triage-and-phase-gates.md`, "TICK ~13:10Z": `vercel env pull`
--   for the minion-hub project, then `psql` against the pooler URL). They were
--   deliberately NOT reconstructed from the checked-in migrations: no
--   `create table crm_contacts` / `crm_activities` and no `*_org_guc` policy
--   for either table exists anywhere in this repository (see the
--   `hub-supabase-schema-not-reproducible` operator note), so a
