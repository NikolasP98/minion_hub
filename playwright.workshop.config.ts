...
const baseURL = process.env.MINION_WORKSHOP_URL;
...
export default defineConfig({
  ...
  testMatch: 'workshop-accessibility.spec.ts',
  workers: 1,
  retries: 0,
  forbidOnly: true,
  use: {
  ...
  outputDir: process.env.MINION_WORKSHOP_RESULTS ?? '.test-artifacts/workshop',
  reporter: [
    ['list'],
    ['json', { outputFile: process.env.MINION_WORKSHOP_REPORT ?? '.test-artifacts/workshop.json' }],
  ],
});
