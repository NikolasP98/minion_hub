...
// Alert delivery is a secondary best-effort path awaited serially in the sync
// loop below — an unavailable monitor must not be able to hold this cron open
// past every other org's turn, so the webhook POST is bounded.
 ...
 *
 * Bounded with AbortSignal.timeout: this is called serially, once per org, in
 * the sync loop below. A monitor endpoint that accepts the connection and
 * never responds would otherwise hold the whole cron open until the platform
 * kills it, and every later org would silently never get synced or alerted.
 */
