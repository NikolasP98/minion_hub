...
describe('GET /api/finances/sync/daily — factory monitor webhook', () => {
  ...
  it('bounds the monitor POST with an AbortSignal so an unresponsive monitor cannot hang the cron', async () => {
    testEnv.FINANCE_ALERT_WEBHOOK_URL = 'https://factory.minion-ai.org/hooks/monitor';
    testEnv.FINANCE_ALERT_WEBHOOK_TOKEN = 'hook-token';
    getJobById.mockResolvedValue({ status: 'failed', error: 'susii login failed: 400' });
    // Simulate an unresponsive monitor: fetch never settles on its own, only
    // reacts to abort. If the route awaited this with no deadline, this test
    // would hang until the surrounding test runner times out.
