...
export const PUT: RequestHandler = async ({ locals, request }) => {
  ...
  const stages = Array.isArray(b.stages)
    ? (b.stages as Array<Record<string, unknown>>).map(
        (s) =>
          ({
            key: String(s.key),
            ...
            ...(s.recipients
              ? { recipients: String(s.recipients) as ReminderStage['recipients'] }
              : {}),
          }) as ReminderStage,
      )
    : undefined;
  ...
  await saveReminderConfig(ctx, {
    ...
    ...(b.inferConfirmation !== undefined
      ? { inferConfirmation: b.inferConfirmation === true }
      : {}),
