...
export const GET: RequestHandler = async ({ request }) => {
  ...
  const totals = { orgs: orgs.length, created: 0, resumed: 0, advanced: 0, errors: 0 };
  for (const { id: orgId } of orgs) {
    try {
      const job = await ensureWhatsAppReconcileJob(orgId);
      if (job.created) totals.created += 1;
      else totals.resumed += 1;
      await advanceBrainCorpusJobNow(job.jobId);
      totals.advanced += 1;
    } catch (cause) {
      totals.errors += 1;
