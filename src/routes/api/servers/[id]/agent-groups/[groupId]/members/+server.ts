...
export const PUT: RequestHandler = async ({ locals, params, request }) => {
  ...
  await setGroupMembers(locals.tenantCtx, params.id!, params.groupId!, agentIds);
  return json({ ok: true });
};
...
export const POST: RequestHandler = async ({ locals, params, request }) => {
  ...
  await addAgentToGroup(locals.tenantCtx, params.id!, params.groupId!, agentId);
...
export const DELETE: RequestHandler = async ({ locals, params, request }) => {
  ...
  const { agentId } = await request.json();
  if (!agentId || typeof agentId !== 'string') throw error(400, 'agentId is required');
  await removeAgentFromGroup(locals.tenantCtx, params.id!, params.groupId!, agentId);
  return json({ ok: true });
};
