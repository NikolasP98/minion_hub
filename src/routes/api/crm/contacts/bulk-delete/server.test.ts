...
it('requires delete permission and performs one service call for all ids', async () => {
  ...
  expect(mocks.softDeleteContacts).toHaveBeenCalledWith(expect.anything(), ids, undefined);
  await expect(response.json()).resolves.toEqual({ ok: true, deleted: 2 });
});

it('passes the owner-scope filter through to the service for an owner-scoped caller', async () => {
  const ids = ['00000000-0000-4000-8000-000000000001'];
  mocks.ownerFilter.mockResolvedValue('profile-1');
  const response = await POST({
    locals: {},
    request: new Request('https://hub.test/api/crm/contacts/bulk-delete', {
      method: 'POST',
  ...
  expect(mocks.softDeleteContacts).toHaveBeenCalledWith(expect.anything(), ids, 'profile-1');
  await expect(response.json()).resolves.toEqual({ ok: true, deleted: 2 });
});
