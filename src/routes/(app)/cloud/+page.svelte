...
<main class="page">
  ...
  {:else if cloudState.error && cloudState.shells.length === 0}
    <div class="error-panel">
      <Cloud size={22} /><strong>{m.cloud_load_failed()}</strong><span>{cloudState.error}</span>
    </div>
  {:else if !selected}
  ...
  {:else}
    <section class="hero">
      <div class="hero-copy">
        ...
        <div class="status-line">
          <span class="status-dot status-{selected.status}"></span><strong>{selected.status}</strong
          ><span>·</span><span>{selected.provider ?? 'exe.dev'}</span><span>·</span><span
    ...
    <section class="spec-grid" aria-label={m.cloud_machine_specs()}>
      <div class="spec-card">
        <Cpu size={16} /><span>{selected.cpu ?? 2}</span><small>{m.cloud_vcpu()}</small>
      </div>
      <div class="spec-card">
        <MemoryStick size={16} /><span>{Math.round(selected.memoryMB / 1024)} GB</span><small
          >{m.cloud_memory()}</small
        >
      </div>
      <div class="spec-card">
        <HardDrive size={16} /><span>{selected.diskGB} GB</span><small>{m.cloud_storage()}</small>
      </div>
      <div class="spec-card">
        <PackageCheck size={16} /><span>{selected.runtimes?.length ?? 1}</span><small
          >{m.cloud_runtimes()}</small
        >
      </div>
    </section>
    ...
    <section class="content-grid">
      <div class="panel access-panel">
        <div class="panel-head">
          <span>{m.cloud_quick_access()}</span><small>{m.cloud_secure_sessions()}</small>
        </div>
        <div class="access-grid">
          {#if data.canConnect}
            <a href={`/cloud/gui${suffix}`}
              ><span class="access-icon"><Monitor size={18} /></span><span
                ><strong>{m.cloud_open_gui()}</strong><small>{m.cloud_open_gui_hint()}</small></span
              ><b>↗</b></a
            >
            <a href={`/cloud/terminal${suffix}`}
              ><span class="access-icon"><SquareTerminal size={18} /></span><span
      ...
      <div class="panel detail-panel">
        <div class="panel-head">
          <span>{m.cloud_runtime_stack()}</span><small>minion-workstation-v1</small>
        </div>
...
<style>
  ...
  .hero {
    ...
    background: linear-gradient(120deg, var(--elevation-2-bg), var(--elevation-1-bg));
  }
  .hero::after {
    content: '';
    position: absolute;
    ...
    background: repeating-linear-gradient(
      90deg,
      transparent 0 47px,
      color-mix(in srgb, var(--hairline) 55%, transparent) 48px
    );
  ...
  .status-line {
    ...
    gap: 0.4rem;
    color: var(--color-muted);
    font: 0.6rem/1 var(--font-mono, monospace);
  ...
  .machine-glyph {
    ...
    border-radius: 50%;
    color: var(--color-accent);
    box-shadow: 0 0 0 0.8rem color-mix(in srgb, var(--color-accent) 3%, transparent);
  }
  .machine-glyph span {
    position: absolute;
    bottom: 0.55rem;
  ...
  .spec-card span {
    color: var(--color-foreground);
    font: 650 0.9rem/1 var(--font-mono, monospace);
  }
  .spec-card small {
    grid-column: 2;
    margin-top: 0.25rem;
  ...
  .panel-head small {
    margin-left: auto;
    color: var(--color-muted);
    font: 0.54rem/1 var(--font-mono, monospace);
  }
  .access-grid {
    display: grid;
  ...
  .runtime-list span {
    ...
    border: 1px solid var(--hairline);
    border-radius: 999px;
    font: 0.58rem/1 var(--font-mono, monospace);
  }
  .runtime-list i {
    width: 0.35rem;
    height: 0.35rem;
