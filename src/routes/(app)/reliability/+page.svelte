<script lang="ts">
  ...
  // there only.
  const GLOBAL_CATEGORIES: ReadonlySet<string> = new Set([
    'gateway',
    'crash',
    'connection',
    'cron',
  ...
  const ORG_CATEGORIES: ReadonlySet<string> = new Set(
    CATEGORIES.filter((c) => !GLOBAL_CATEGORIES.has(c)),
  );
  ...
  const anyFilterVisible = $derived(
    tabFilterVis.severity || tabFilterVis.category || tabFilterVis.mode,
  );
  ...
  function buildSpark(vals: number[], dir: 1 | -1): SparkData | undefined {
    ...
    if (st.series.length < 4) {
      statusColor = C.muted;
      statusLabel = m.reliability_tipInsufficient();
    } else if (badMag > 3) {
      statusColor = C.destructive;
      statusLabel = m.reliability_tipOOC();
    } else if (badMag > 2) {
      statusColor = C.warning;
      statusLabel = m.reliability_tipNearLimit();
    } else if (!vsRollGood && badMag > 0.5) {
      statusColor = C.warning;
      statusLabel = m.reliability_tipDrift();
    } else {
      statusColor = C.success;
      statusLabel = m.reliability_tipInControl();
    }
  ...
  const statItems = $derived.by(() => {
    ...
    const rangeHours = Math.max(
      (reliability.dateRange.to - reliability.dateRange.from) / 3_600_000,
      1 / 60,
    );
    const rate = total / rangeHours;
    const rateStr =
      rate >= 100
        ? compact(Math.round(rate))
        : rate >= 10
    ...
    const healthSeries = withTotal.map((b) =>
      Math.max(
        0,
        Math.min(100, (1 - (W_CRIT * b.crit + W_HIGH * b.high + W_MED * b.med) / b.total) * 100),
      ),
    );
    ...
    const dataSrc =
      !hasActiveFilters && reliability.activity
        ? m.reliability_tipFleet()
        : m.reliability_tipFiltered();
    ...
    const latColor =
      p95Ms === null ? C.muted : p95Ms < 200 ? C.success : p95Ms < 1000 ? C.warning : C.destructive;
    const loopColor =
      loopP99 === null
        ? C.muted
        : loopP99 < 50
    ...
    const avgCostUsd =
      usageTot && usageTot.calls ? usageTot.costMicroUsd / 1_000_000 / usageTot.calls : null;
    ...
    const healthItem = {
      key: 'health',
      Icon: Gauge,
      color: healthSpark?.statusColor ?? rampGood(healthPct),
      label: m.reliability_kpiHealth(),
      value: healthPct ?? '—',
      unit: healthPct ? '%' : '',
      subtext: total
        ? m.reliability_kpiHealthSub({
            crit: compact(crit),
            high: compact(high),
            med: compact(med),
          })
    ...
    const errorItem = {
      key: 'error',
      Icon: TrendingDown,
      color: errorSpark?.statusColor ?? rampBad(errorPct),
      label: m.reliability_kpiErrorRate(),
      value: errorPct ?? '—',
      unit: errorPct ? '%' : '',
      subtext: total
        ? `${compact(fail)} ${m.reliability_kpiOfTotal({ total: compact(total) })}`
        : '',
      detail: {
        label: m.reliability_kpiErrorRate(),
    ...
    const criticalItem = {
      key: 'critical',
      Icon: AlertCircle,
      color: critSpark?.statusColor ?? (crit > 0 ? C.destructive : C.success),
    ...
    const rateItem = {
      ...
      value: total ? rateStr : '—',
      unit: total ? m.reliability_kpiPerHour() : '',
    ...
    const heartbeatItem = {
      key: 'heartbeat',
      Icon: HeartPulse,
      color: hbSpark?.statusColor ?? rampGood(hbPct),
    ...
    const toolItem = {
      key: 'tool',
      Icon: Wrench,
      color: toolSpark?.statusColor ?? rampGood(toolPct),
    ...
    return {
      ...
      overview: [
        ...
        noiseItem,
        {
          ...
          Icon: Timer,
          color: latColor,
          label: m.reliability_kpiLatencyP95(),
          value: p95Ms ?? '—',
          unit: p95Ms !== null ? 'ms' : '',
          subtext: perf
            ? m.reliability_kpiLatencyP99Sub({ p99: Math.round(perf.latencyMs.p99) })
            : '',
        },
        {
          key: 'throughput',
          Icon: Activity,
          color: C.cyan,
          label: m.reliability_kpiThroughput(),
          value: thrStr,
          unit: throughput !== null ? m.reliability_kpiPerSec() : '',
        ...
        },
        {
          ...
          Icon: Gauge,
          color: loopColor,
          label: m.reliability_kpiEventLoop(),
          value: loopP99 ?? '—',
          unit: loopP99 !== null ? 'ms' : '',
          subtext: perf
            ? m.reliability_kpiLoopSub({ p50: Math.round(perf.eventLoopDelayMs.p50) })
            : '',
        },
        rateItem,
      ],
      ...
      agents: [
        ...
        heartbeatItem,
        {
          key: 'timeout',
          Icon: Timer,
          color: timeoutPct === null ? C.muted : rampBad(timeoutPct, 2, 10),
          label: m.reliability_kpiToolTimeout(),
          value: timeoutPct ?? '—',
          unit: timeoutPct !== null ? '%' : '',
        ...
        },
        {
          key: 'auth',
          Icon: ShieldAlert,
          color: authPct === null ? C.muted : rampBad(authPct, 1, 5),
        ...
        },
        {
          ...
          Icon: Zap,
          color: C.accent,
          label: m.reliability_kpiLlmCalls(),
          value: callN === null ? '—' : compact(callN),
          unit: '',
        ...
        },
        {
          ...
          Icon: DollarSign,
          color: C.muted,
          label: m.reliability_kpiAvgCost(),
          value: avgCostUsd === null ? '—' : formatUsd(avgCostUsd),
          unit: '',
          subtext: '',
        },
        {
  ...
  let timelineOptions: EChartsOption = $derived.by(() => {
    ...
    const visibleCategories =
      effCategories.size > 0 ? CATEGORIES.filter((c) => effCategories.has(c)) : CATEGORIES;
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          const rows = nonZero
            .map(
              (p) =>
                `<tr><td style="padding-right:var(--space-3,12px)">${p.marker}${p.seriesName}</td><td style="text-align:right;font-weight:600">${p.value[1]}</td></tr>`,
            )
            .join('');
  ...
  const topEventTypes = $derived.by(() => {
    ...
    return [...types].map((type) => ({
      type,
      color: CATEGORY_COLORS[type] ?? TOP_EVENT_FALLBACK_COLOR,
    }));
  });
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          return `<div style="font-size: var(--font-size-label, 11px)">
						...
						<div style="display:flex;align-items:center;gap:var(--space-2,8px);margin:var(--space-0-5,2px) 0;color:var(--color-muted-foreground);font-size: var(--font-size-caption, 10px)">
  ...
  function handleSankeyClick(params: unknown) {
    ...
    if (id.startsWith('m:')) toggleFailureMode(id.slice(2));
      ...
      const c = id.slice(2);
      if ((CATEGORIES as readonly string[]).includes(c)) toggleCategory(c);
    } else if (id.startsWith('s:')) {
      const s = id.slice(2);
      if ((SEVERITIES as readonly string[]).includes(s)) toggleSeverity(s);
    }
  }
  ...
  let sankeyOptions: EChartsOption = $derived.by(() => {
    ...
    for (const r of flowRows) {
      ...
      catsUsed.add(cat);
      sevsUsed.add(sevKey);
      mc.set(`${mode}\0${cat}`, (mc.get(`${mode}\0${cat}`) ?? 0) + r.count);
      cs.set(`${cat}\0${sevKey}`, (cs.get(`${cat}\0${sevKey}`) ?? 0) + r.count);
    }
    ...
    for (const mode of modesUsed) {
      const color =
        mode === OTHER
          ? cssVar('--color-text-disabled', '#64748b')
          : (CATEGORY_COLORS[modeType.get(mode) ?? ''] ?? cssVar('--color-accent', '#3b82f6'));
      addNode(`m:${mode}`, color, 'left');
    }
    for (const cat of catsUsed)
      addNode(
        `c:${cat}`,
        CATEGORY_COLORS[cat] ?? cssVar('--color-text-tertiary', '#64748b'),
        'right',
      );
    for (const sevKey of sevsUsed)
      addNode(
        `s:${sevKey}`,
        SEVERITY_COLORS[sevKey] ?? cssVar('--color-neutral', '#64748b'),
        'right',
      );

    const links: Array<{ source: string; target: string; value: number }> = [];
    for (const [k, v] of mc) {
      const [mode, cat] = k.split('\0');
      links.push({ source: `m:${mode}`, target: `c:${cat}`, value: v });
    }
    for (const [k, v] of cs) {
      const [cat, sevKey] = k.split('\0');
      links.push({ source: `c:${cat}`, target: `s:${sevKey}`, value: v });
    }
...
<PageShell archetype="dashboard" scroll="none" labelledBy="reliability-title">
    ...
    {#snippet actions()}
      {#if scopeActive}
        ...
        <div class="flex items-center gap-1" role="group" aria-label={m.reliability_scopeLabel()}>
          <span
            class="text-xs font-semibold uppercase tracking-wider text-muted-foreground mr-0.5 hidden sm:inline"
            >{m.reliability_scopeLabel()}</span
          >
          <Button
            type="button"
            onclick={() => setScope('all')}
            variant={scopeMode === 'all' ? 'primary' : 'outline'}
            size="sm"
            class="rounded-full"
            aria-pressed={scopeMode === 'all'}>{m.reliability_scopeAll()}</Button
          >
          <Button
            type="button"
            onclick={() => setScope('global')}
            variant={scopeMode === 'global' ? 'primary' : 'outline'}
            size="sm"
            class="rounded-full"
            aria-pressed={scopeMode === 'global'}>{m.reliability_scopeGlobal()}</Button
          >
          <Button
            type="button"
            onclick={() => setScope('org')}
            variant={scopeMode === 'org' ? 'primary' : 'outline'}
            size="sm"
            class="rounded-full"
            aria-pressed={scopeMode === 'org'}>{m.reliability_scopeOrg()}</Button
          >
        </div>
      {/if}
      {#if activeTab !== 'architecture'}
        <!-- Architecture is a live-now snapshot — date range + event refresh don't
				     apply there (the tab has its own probe refresh in its legend). -->
        <DateRangeControls
          from={relWindow.from}
          to={relWindow.to}
          withTime
  ...
  <div
    class="shrink-0 relative z-[var(--layer-popover)] border-b border-border bg-bg2/80 backdrop-blur-sm"
  >
    <Tabs
      id="reliability-tabs"
      tabs={tabItems}
    ...
    {#if summary && anyFilterVisible}
      <div class="flex items-center gap-2 flex-wrap px-4 pt-2 pb-2.5 border-t border-border/60">
        ...
        {/if}
        {#if hasActiveFilters}
          <Button variant="ghost" size="sm" class="shrink-0 text-xs" onclick={clearAllFilters}>
      ...
      <AsyncBoundary state={pageState} class="h-full">
        <div class="flex flex-col gap-3 {activeTab === 'architecture' ? 'h-full' : ''}">
          {#if activeTab === 'overview'}
            ...
            <div class="grid grid-cols-2 gap-3 max-[800px]:grid-cols-1">
              <div class="surface-2 rounded-lg overflow-hidden">
                  ...
                  <div class="ml-auto flex items-center gap-2 flex-wrap justify-end">
                    {#each topEventTypes as t (t.type)}
                      ...
                      >
                        ...
                        <span
                          class="text-xs font-medium {selectedCategories.has(t.type)
                            ? 'text-foreground'
                            : ''}">{t.type}</span
                        >
                      </Button>
                ...
                <div class="relative overflow-hidden">
                  ...
                  <Chart
                    ...
                    height="300px"
                    onItemClick={handleTopEventClick}
                  />
                </div>
              </div>
              ...
              <div class="surface-2 rounded-lg overflow-hidden">
                <div class="flex items-center gap-2 px-4 py-2 border-b border-border bg-bg3/20">
                  ...
                  <span
                    class="text-xs font-semibold uppercase tracking-widest text-muted-foreground"
                    >{m.reliability_severityDistribution()}</span
                  >
                </div>
                <div class="relative overflow-hidden">
                  ...
                  <Chart
                    ...
                    height="300px"
                    onItemClick={handleSeverityClick}
                  />
                </div>
              </div>
            </div>
            ...
            <!-- ── Gateway Health ───────────────────────────────────────────────── -->
            {#if serverId}<GatewayHealthPanel {serverId} />{/if}
  ...
  {#if hoveredKpi && kpiTipPos}
    ...
    >
      <div class="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-1.5">
        {hoveredKpi.label}
      </div>
      ...
      {#if kpiView === 'visuals' && hoveredKpi.spark}
        ...
        <div class="flex items-center justify-between mb-1">
          <span class="text-xs text-muted-strong uppercase tracking-wide"
            >{m.reliability_tipTrend()}</span
          >
          <span class="text-xs font-semibold uppercase tracking-wide" style:color={s.statusColor}
            >{s.statusLabel}</span
          >
        </div>
        <KpiSparkline
        ...
        <div class="flex items-center gap-3 mt-1 text-xs text-muted-strong">
          <span class="flex items-center gap-1"
            ><span class="inline-block w-3 h-[2px] opacity-40" style:background={s.color}
            ></span>{m.reliability_tipRaw()}</span
          >
          <span class="flex items-center gap-1"
            ><span class="inline-block w-3 h-[2px]" style:background={s.color}
            ></span>{m.reliability_tipRollingShort()}</span
          >
          <span class="flex items-center gap-1"
            ><span
              class="inline-block w-3 border-t border-dashed opacity-60"
        ...
        <div class="grid grid-cols-2 gap-x-3 gap-y-0.5 mt-1.5 text-xs tabular-nums">
          <div class="flex justify-between gap-2">
            <span class="text-muted-strong">{m.reliability_tipRolling()}</span><span
              >{s.rollAvgVal.toFixed(0)}%</span
            >
          </div>
          <div class="flex justify-between gap-2">
            <span class="text-muted-strong">{m.reliability_tipVsAvg()}</span><span
              class="font-semibold"
              style:color={s.vsRollGood ? C.success : C.warning}
              >{s.current - s.rollAvgVal >= 0 ? '+' : ''}{(s.current - s.rollAvgVal).toFixed(1)} · {s.trendLabel}</span
            >
          </div>
          <div class="flex justify-between gap-2">
            <span class="text-muted-strong">μ±3σ</span><span
              >{s.lcl.toFixed(0)}–{s.ucl.toFixed(0)}%</span
            >
          </div>
          <div class="flex justify-between gap-2">
            <span class="text-muted-strong">z</span><span
              >{s.z >= 0 ? '+' : ''}{s.z.toFixed(1)}σ</span
            >
          </div>
        </div>
      {:else}
        <div class="text-xs text-muted-strong uppercase tracking-wide mb-0.5">
          {m.reliability_tipFormula()}
        </div>
        <div class="text-foreground text-sm mb-2 overflow-hidden">
          <MathFormula tex={hoveredKpi.texFormula} />
        </div>
        <div class="text-xs text-muted-strong uppercase tracking-wide mb-0.5">
          {m.reliability_tipResult()}
        </div>
        <div class="text-foreground text-sm overflow-hidden">
          <MathFormula tex={hoveredKpi.texValues} />
        </div>
        {#if hoveredKpi.note}
          <div
            class="mt-2 pt-1.5 border-t border-border/60 text-xs text-muted-strong leading-snug whitespace-normal"
          >
            {hoveredKpi.note}
          </div>
        {/if}
      ...
      {#if kpiViews.length > 1}
        ...
        <div
          class="mt-2.5 pt-2 border-t border-border/60 flex items-center justify-center gap-1.5"
          aria-hidden="true"
        >
          {#each kpiViews as _view, i (i)}
            <span
              class="h-1.5 rounded-full {i === kpiStep
                ? 'w-4 bg-accent'
