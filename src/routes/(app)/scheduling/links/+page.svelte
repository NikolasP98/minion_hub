...
<div class="flex flex-col h-full min-h-0">
	<PageHeader title={m.sched_links_title()} subtitle={m.sched_dashboard_subtitle()}>
		{#snippet leading()}
				...
				<div class="mt-3">
					...
					<div class="flex flex-wrap gap-2 mt-1">
						{#each data.eventTypes as e (e.id)}
							<button type="button" class="chip {chosen.includes(e.id) ? 'chip-on' : ''}" onclick={() => toggle(e.id)}>
								{e.title}
							</button>
						{/each}
					</div>
			...
			{#each data.links as link (link.id)}
				<Card padding="md">
					<div class="flex items-center gap-3 flex-wrap">
						...
						<button class="act" onclick={() => copy(link.slug)} title={m.sched_link_copy()}>
							<Copy size={15} /> {copied === link.slug ? m.sched_link_copied() : ''}
						</button>
