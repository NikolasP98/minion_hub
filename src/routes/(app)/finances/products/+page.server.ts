...
export const load: PageServerLoad = async ({ locals, depends }) => {
  ...
  const costs = await costForProducts(ctx, products.map((p) => p.id));
  const enriched = products.map((p) => {
    ...
    if (mask) return { ...p, cost: null, margin: null, marginPct: null, costable, partial, costMasked: true };
    ...
    return { ...p, cost, margin, marginPct, costable, partial, costMasked: false };
  });
