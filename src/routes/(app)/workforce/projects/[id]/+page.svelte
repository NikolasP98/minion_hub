<script lang="ts">
  ...
  import {
    pipelineColumns,
    type PipelineColumnBucket,
    type KanbanIssueLike,
  } from '$lib/components/workforce/pipeline-columns';
  ...
  function pipelineColLabel(col: PipelineColumnBucket<IssueRow>): string {
    switch (col.kind) {
      case 'intake':
        return m.workforce_kanban_intake();
      case 'step':
        return selectedPipeline?.steps[col.stepIndex!]?.label ?? '—';
      case 'review':
        return m.workforce_kanban_inReview();
      case 'done':
        return m.workforce_kanban_done();
      case 'blocked':
        return m.workforce_kanban_blocked();
    }
  }
    ...
    try {
      const res = await fetch(`/api/projects/${data.project.id}/tasks`, {
        ...
        body: JSON.stringify({
          ...
          isMilestone,
          status: isMilestone ? 'todo' : 'backlog',
        }),
      });
      if (res.ok) {
        newTaskTitle = '';
  ...
  async function patchProject(body: Record<string, unknown>) {
    ...
    try {
      const res = await fetch(`/api/projects/${data.project.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ ...body, expectedUpdatedAt: data.project.updatedAt }),
      });
      if (res.status === 409) toastWarning(m.shared_staleWrite());
      await invalidate('projects:detail');
    } finally {
  ...
  <div class="flex-1 min-h-0 overflow-auto p-4 flex flex-col gap-4">
    ...
    <div class="add">
      ...
      <Button
        ...
        title={canAct('projects', 'edit') ? undefined : m.no_permission()}
        onclick={() => addTask(false)}><Plus size={15} /> Task</Button
      >
      <Button
        variant="ghost"
        class="btn ghost"
        disabled={busy || !newTaskTitle.trim() || !canAct('projects', 'edit')}
        title={canAct('projects', 'edit') ? undefined : m.no_permission()}
        onclick={() => addTask(true)}><Flag size={iconSizes.sm} /> Milestone</Button
      >
    </div>
      ...
      {#each COLUMNS as col (col)}
        <div class="col">
          <header class="col-head">
            {colLabel[col]}
            <span class="cnt">{workTasks.filter((t) => t.status === col).length}</span>
          </header>
          <div class="col-body">
              ...
              <div class="tcard p-{t.priority}">
                ...
                <div class="tmeta">
                  ...
                  <Select
                    ...
                    title={canAct('projects', 'edit') ? undefined : m.no_permission()}
                    onchange={(value) =>
                      patchTask(t.id, { assigneePartyId: String(value) || null }, t.updatedAt)}
                  >
                    ...
                    {/if}
                    {#each assignOptions as o (o.id)}<option
                        value={o.id}
                        disabled={!!o.agent && !data.workforceAvailable}
                        >{o.label}{#if o.agent && !data.workforceAvailable}
                          — {m.workforce_projects_agentUnavailable()}{/if}</option
                      >{/each}
                  </Select>
                </div>
                <Select
    ...
    <section class="card exec" class:offline={!data.workforceAvailable}>
          ...
          <span>
            {#if data.workforceAvailable}
              <a class="btn ghost sm" href={`/workforce/projects/${data.project.id}/pipelines`}>
                <Workflow size={iconSizes.sm} />
                {m.workforce_pipelines_title()}
              </a>
            {:else}
              <span class="btn ghost sm disabled" aria-disabled="true"
                ><Workflow size={iconSizes.sm} /> {m.workforce_pipelines_title()}</span
              >
            {/if}
            <Button
              variant="ghost"
      ...
      {#if data.harnessRoster.length}
        ...
        <div class="roster">
          {#each data.harnessRoster as harness (harness.agentId)}
            <a
              class="agent-card"
              class:disabled={!data.workforceAvailable}
              href={data.workforceAvailable ? `/workforce/agents/${harness.agentId}` : undefined}
              aria-disabled={!data.workforceAvailable}
            >
              <span class="agent-name">{harness.name}</span>
              <span class="t-caption font-mono"
                >{harness.activePrimary?.model ?? m.workforce_harness_offline()}</span
              >
              <span class="t-caption"
                >r{harness.revision ?? '?'} · {harness.performance.successRate == null
                  ? '—'
                  : `${Math.round(harness.performance.successRate * 100)}%`}</span
              >
            </a>
          {/each}
        </div>
      ...
      {:else if data.execution}
        <div class="exec-meta">
          ...
          <span class="t-caption"
            >{data.execution.project.workspaces.length} workspace{data.execution.project.workspaces
              .length !== 1
              ? 's'
              : ''}</span
          >
        ...
        {#if data.execution.project.workspaces.length}
          <div class="ws-list">
            {#each data.execution.project.workspaces as ws (ws.id)}
              <div class="ws">
                <span class="ws-name"
                  >{ws.name}{#if ws.isPrimary}<span class="primary">primary</span>{/if}</span
                >
        ...
        {#if data.execution.issues.length}
          {#if pipelineBoard && selectedPipeline}
            ...
            {/if}
            <div
              class="board exec-board"
              style={`grid-template-columns: repeat(${pipelineBoard.length}, minmax(140px, 1fr));`}
            >
              {#each pipelineBoard as col (col.key)}
                {@const step = col.stepIndex != null ? selectedPipeline.steps[col.stepIndex] : null}
                <div class="col">
                  <header class="col-head">
                    {pipelineColLabel(col)} <span class="cnt">{col.issues.length}</span>
                  </header>
                  <div class="col-body">
                        ...
                        <div class="tmeta">
                          ...
                          {#if step && col.stepIndex != null && col.stepIndex > 0}
                            {#if step.participant.type === 'user'}
                              <span class="step-chip hitl"
                                >{step.label} · {m.workforce_kanban_you()}</span
                              >
                            {:else}
                              <span class="step-chip review"
                                >{step.label} · {wfAgent(step.participant.agentId ?? null)}</span
                              >
                            {/if}
          ...
          {:else}
            <div class="board exec-board">
              {#each COLUMNS as col (col)}
                ...
                <div class="col">
                  <header class="col-head">
                    {colLabel[col]} <span class="cnt">{colIssues.length}</span>
                  </header>
                  <div class="col-body">
                      ...
                      <a class="tcard iss-card" href={`/workforce/issues/${iss.id}`}>
                        ...
                        <div class="tmeta">
                          ...
                          {:else if iss.status === 'in_review' && iss.assigneeAgentId}
                            <span class="step-chip review"
                              >review · {wfAgent(iss.assigneeAgentId)}</span
                            >
                          {:else if iss.assigneeAgentId}
                            <span class="step-chip">{wfAgent(iss.assigneeAgentId)}</span>
                          {:else if iss.assigneeUserId}
      ...
      {:else if data.workforceProjectId}
        <p class="t-caption broken">
          Linked workforce project unavailable (backend down or project removed).
        </p>
      {:else}
        <div class="link-row">
    ...
    <section class="card ts">
      ...
      <div class="ts-add">
        ...
        <Button variant="ghost" class="btn" disabled={busy || !data.selfPartyId} onclick={logTime}
          >Log</Button
        >
      </div>
      <div class="ts-list">
...
<style>
  ...
  .ring {
    ...
    display: grid;
    place-items: center;
    background: conic-gradient(var(--color-accent) calc(var(--pct) * 1%), var(--color-bg3) 0);
    flex: 0 0 auto;
  }
  .ring span {
  ...
  .summary-meta :global([data-part='select']) {
  ...
  .ms-chip.done {
    color: var(--color-success, var(--color-success-border));
  }
  .add {
    display: flex;
    gap: var(--space-2);
  ...
  .col {
    background: var(--color-bg2, var(--color-bg3));
    border: 1px solid var(--hairline);
    border-radius: var(--radius-lg);
    min-height: 4rem;
  }
  ...
  .step-chip.hitl {
    ...
    border-color: color-mix(
      in oklab,
      var(--color-warning, var(--color-warning-border)) 40%,
      transparent
    );
  }
  .link-row {
  ...
  .ts-row {
    display: grid;
    grid-template-columns: 6rem 5rem 7rem 1fr auto;
    gap: var(--space-2);
    align-items: center;
