<script lang="ts">
  ...
  import {
    ...
    daysBetween,
    type Period,
  } from '$lib/components/dashboard/date-range';
  ...
  const kpis = $derived(
    data.kpis
      ? [
          {
            id: 'spend',
            label: m.ads_kpi_spend(),
            value: fmtMoney(data.kpis.spend),
            delta: data.kpis.deltaPct.spend,
          },
          {
            id: 'conversations',
            label: m.ads_kpi_conversations(),
            value: fmtInt(convo.started),
            delta: null,
          },
          {
            id: 'cost-per-convo',
          ...
          },
          {
            id: 'ctr',
            label: m.ads_kpi_ctr(),
            value: fmtPct(data.kpis.ctr),
            delta: data.kpis.deltaPct.ctr,
          },
          {
            id: 'impressions',
          ...
          {
            id: 'reach',
            label: m.ads_kpi_reach(),
            value: fmtInt(data.kpis.reach),
            delta: data.kpis.deltaPct.reach,
          },
          {
            id: 'clicks',
            label: m.ads_kpi_clicks(),
            value: fmtInt(data.kpis.clicks),
            delta: data.kpis.deltaPct.clicks,
          },
          {
            id: 'cpc',
            label: m.ads_kpi_cpc(),
            value: fmtMoney(data.kpis.cpc),
            delta: data.kpis.deltaPct.cpc,
          },
        ]
      : [],
  );
  ...
  const efficiencyOpts = $derived({
    ...
    series: [
      ...
      },
      {
        ...
        data: series.map((r) =>
          r.impressions > 0 ? Math.round((r.clicks / r.impressions) * 10000) / 100 : 0,
        ),
      },
    ],
  } satisfies EChartsOption);
  ...
  const campaignOpts = $derived({
    ...
    yAxis: {
      type: 'category',
      data: topCampaigns.map((r) => r.campaignName ?? r.campaignId ?? '—'),
      inverse: true,
    },
    ...
    series: [
      {
        ...
        data: topCampaigns.map((r) => Math.round(r.spend * 100) / 100),
      },
    ],
  } satisfies EChartsOption);
  ...
  {#if kpiById.has(id)}
    ...
    {#if k}
      <div class="kpi">
        ...
        <div
          class="kpi-delta"
          class:pos={k.delta !== null && k.delta >= 0}
          class:neg={k.delta !== null && k.delta < 0}
        >
          {fmtDelta(k.delta)}
        </div>
      </div>
          ...
          <tbody>
            {#each topCampaigns as r (r.campaignId ?? r.campaignName)}
              <tr>
                <td class="cname">
                  {#if r.campaignId}
                    <a href="/socials/campaigns/{encodeURIComponent(r.campaignId)}"
                      >{r.campaignName ?? r.campaignId}</a
                    >
                  {:else}
                    {r.campaignName ?? '—'}
                  {/if}
                ...
                <td class="num"
                  >{r.costPerConversation === null ? '—' : fmtMoney(r.costPerConversation)}</td
                >
        ...
        <ul class="post-list">
          {#each data.posts as post (post.postId)}
            <li class="post-row">
              {#if post.thumbFileId}
                <img
                  src="/api/files/{post.thumbFileId}/raw"
                  loading="lazy"
                  alt=""
                  width="36"
                  height="36"
                ...
                />
              {/if}
              <span class="post-platform" data-platform={post.platform ?? ''}
                >{post.platform === 'ig' ? m.ads_platform_ig() : m.ads_platform_fb()}</span
              >
  ...
  <div class="flex-1 min-h-0 overflow-auto p-4">
    <div class="w-full max-w-6xl mx-auto">
      {#if !data.hasConnection}
        <EmptyState
          icon={Megaphone}
          title={m.ads_empty_title()}
          description={m.ads_empty_dashboard_desc()}
        >
          {#snippet action()}
            <Button variant="primary" size="sm" href="/api/meta/oauth/start"
              >{m.ads_connect_meta()}</Button
            >
          {/snippet}
        </EmptyState>
      {:else}
