<script lang="ts">
  ...
  onMount(() => {
    ...
    if (connected === '1') toastSuccess(m.ads_toast_connected());
      ...
      toastError(m.ads_toast_connect_error({ reason: qp.get('reason') ?? '—' }));
  });

  function statusLabel(status: string): string {
    switch (status) {
      case 'active':
        return m.ads_status_active();
      case 'expiring':
        return m.ads_status_expiring();
      case 'expired':
        return m.ads_status_expired();
      case 'revoked':
        return m.ads_status_revoked();
      default:
        return status;
    }
  }
  function assetKindLabel(kind: string): string {
    switch (kind) {
      case 'page':
        return m.ads_asset_kind_page();
      case 'ig':
        return m.ads_asset_kind_ig();
      case 'ad_account':
        return m.ads_asset_kind_ad_account();
      default:
        return kind;
    }
  }
  ...
  const igAsset = $derived(
    igConn ? data.assets.find((a) => a.connectionId === igConn.id) : undefined,
  );
  ...
  const jobColumns: DataColumn<Job>[] = [
    ...
    {
      key: 'counts',
      label: m.ads_sync_col_counts(),
      custom: true,
      accessor: (j) => countsSummary(j.counts),
      sortable: false,
    },
    { key: 'error', label: m.ads_sync_col_error(), custom: true, accessor: (j) => j.error ?? '' },
    {
      key: 'finished',
      label: m.ads_sync_col_finished(),
      custom: true,
      accessor: (j) => j.finishedAt,
      sortFn: (a, b) => dateOf(a.finishedAt) - dateOf(b.finishedAt),
      exportValue: (j) => (j.finishedAt ? new Date(j.finishedAt).toISOString().slice(0, 10) : ''),
    },
  ];
</script>
      ...
      <section class="card">
        ...
        {#if fbConn}
          <div class="conn-row">
            <div class="meta-row">
              ...
              <span class="status-pill" data-status={fbConn.status}
                >{statusLabel(fbConn.status)}</span
              >
            </div>
            {#if fbConn.grantedScopes.length > 0}
              <div class="meta-row">
                ...
                <span class="scopes">
                  {#each fbConn.grantedScopes as scope (scope)}<span class="scope-chip"
                      >{scope}</span
                    >{/each}
                </span>
              </div>
        ...
        {:else}
          <EmptyState
            compact
            icon={Plug}
            title={m.ads_connect_meta()}
            description={m.ads_connect_meta_desc()}
          >
            {#snippet action()}
              <Button
                variant="primary"
      ...
      <section class="card">
        ...
        {#if igConn}
          <div class="conn-row">
            <div class="meta-row">
              <span class="t-caption">{m.ads_connection_status()}</span>
              <span class="status-pill" data-status={igConn.status}
                >{statusLabel(igConn.status)}</span
              >
            </div>
            <div class="meta-row">
        ...
        {:else}
          <EmptyState
            compact
            icon={Plug}
            title={m.ads_connect_instagram()}
            description={m.ads_connect_instagram_desc()}
          >
            {#snippet action()}
              <Button
                variant="primary"
...
<style>
  ...
  .status-pill[data-status='failed'] {
    background: color-mix(
      in srgb,
      var(--color-destructive, var(--color-danger-fg)) 12%,
      transparent
    );
