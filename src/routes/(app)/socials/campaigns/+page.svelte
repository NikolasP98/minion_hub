<script lang="ts">
  ...
  const rowId = (r: Row) =>
    r.adId != null ? `a:${r.adId}` : r.adsetId != null ? `s:${r.adsetId}` : `c:${r.campaignId}`;
  ...
  const columns: DataColumn<Row>[] = [
    ...
    {
      key: 'name',
      label: m.ads_col_campaign(),
      custom: true,
      accessor: nameOf,
      exportValue: nameOf,
      sortFn: (a, b) => nameOf(a).localeCompare(nameOf(b)),
      width: 300,
    },
    {
      key: 'spend',
    ...
    {
      key: 'reach',
      label: m.ads_col_reach(),
      align: 'right',
      numeric: true,
      custom: true,
      accessor: (r) => r.reach,
      sortFn: (a, b) => a.reach - b.reach,
      width: 110,
    },
    {
      key: 'clicks',
    ...
    },
    {
      ...
      money: true,
      label: m.ads_col_cost_per_convo(),
      align: 'right',
      numeric: true,
      custom: true,
      accessor: (r) => r.costPerConversation ?? Infinity,
      sortFn: (a, b) => (a.costPerConversation ?? Infinity) - (b.costPerConversation ?? Infinity),
      width: 130,
    },
  ];
  ...
  {#if !data.hasConnection}
    <div class="flex-1 min-h-0 overflow-auto p-4">
      <EmptyState
        ...
        description={m.ads_empty_campaigns_desc()}
      />
    </div>
  {:else}
    <DataTable
      ...
      tableId="socials.campaigns"
      idColumn={{ value: (r) => r.campaignId }}
      titleColumn={{
        key: 'name',
        href: (r) =>
          r.campaignId != null
            ? `/socials/campaigns/${encodeURIComponent(r.campaignId)}`
            : r.postId
              ? `/socials/posts/${encodeURIComponent(r.postId)}`
              : '',
      }}
    >
      {#snippet toolbar()}
        <label class="date-label">
          ...
          <input
            type="date"
            bind:value={fromDate}
            oninput={() => navigateRange(fromDate, toDate)}
          />
        </label>
        <label class="date-label">
      ...
      {#snippet cell(r: Row, col: DataColumn<Row>)}
        ...
        {:else if col.key === 'name'}
          <span class="truncate block" class:font-medium={r.adId == null && r.adsetId == null}
            >{nameOf(r)}</span
          >
        ...
        {:else if col.key === 'costPerConversation'}
          <span class="tabular-nums"
            >{r.costPerConversation == null ? '—' : fmtMoney(r.costPerConversation)}</span
          >
        {/if}
      {/snippet}
    </DataTable>
...
<style>
  ...
  .social-campaigns-page :global(.preset) {
    padding: var(--space-1) var(--space-2);
    font-size: var(--font-size-caption);
  ...
  .social-campaigns-page :global(.preset):hover {
    background: var(--color-card);
    color: inherit;
  }
  .row-thumb {
