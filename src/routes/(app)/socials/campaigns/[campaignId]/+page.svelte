<script lang="ts">
	...
	const kpis = $derived([
		...
		{ id: 'impressions', label: m.ads_kpi_impressions(), value: fmtInt(campaign.totals.impressions) },
	...
	const spendOpts = $derived({
		...
		xAxis: { type: 'category', data: campaign.spendSeries.map((r) => r.date), axisLabel: { hideOverlap: true } },
		yAxis: { type: 'value' },
		series: [
			{
	...
	const adColumns: DataColumn<AdRow>[] = [
		...
		{ key: 'name', label: m.ads_col_ad(), custom: true, accessor: (r) => r.adName ?? '—', exportValue: (r) => r.adName ?? '', sortFn: (a, b) => (a.adName ?? '').localeCompare(b.adName ?? ''), width: 260 },
...
<svelte:head><title>{campaign.campaignName ?? m.ads_nav_campaigns()} · {m.nav_ads()}</title></svelte:head>

<div class="flex flex-col h-full min-h-0">
	<PageHeader title={campaign.campaignName ?? m.ads_nav_campaigns()} subtitle={m.ads_campaign_detail_subtitle()}>
