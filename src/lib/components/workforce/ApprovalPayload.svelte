    ...
    {#if payload.adapterType}
      <div class="flex items-center gap-2">
        ...
        <span class="font-mono text-xs bg-muted px-1.5 py-0.5 rounded"
          >{String(payload.adapterType)}</span
        >
      </div>
    {/if}
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="flex flex-wrap gap-1.5">
          {#each payload.desiredSkills.filter(Boolean) as skill (skill)}
            <span
              class="rounded bg-muted px-1.5 py-0.5 font-mono text-[length:var(--font-size-caption)] text-muted-foreground"
              >{skill}</span
            >
          {/each}
        </div>
...
{:else if type === 'budget_override_required'}
  ...
  {@const observedAmount =
    typeof payload.observedAmount === 'number' ? payload.observedAmount : null}
  <div class="mt-3 space-y-1.5 text-sm">
...
{:else if type === 'request_board_approval'}
  ...
  <div class="mt-4 space-y-3.5 text-sm">
    {#if title}
      <div class="space-y-1">
        <p
          class="text-[length:var(--font-size-caption)] font-medium uppercase tracking-[0.08em] text-muted-foreground"
        >
          Title
        </p>
    ...
    {#if recommendedAction}
      <div class="rounded-lg border border-warning/20 bg-warning/10 px-3.5 py-3">
        <p
          class="text-[length:var(--font-size-caption)] font-medium uppercase tracking-[0.08em] text-warning"
        >
          Recommended action
        </p>
    ...
    {#if proposedComment}
      <div class="space-y-1.5">
        ...
        <pre
          class="max-h-48 overflow-auto rounded-lg border border-border/60 bg-muted/50 px-3.5 py-3 font-mono text-xs leading-5 text-muted-foreground whitespace-pre-wrap">{proposedComment}</pre>
      </div>
    {/if}
  </div>
{:else if type === 'approve_ceo_strategy'}
  ...
  <div class="mt-3 space-y-1.5 text-sm">
    ...
    {#if plan}
      <div
        class="mt-2 max-h-64 overflow-y-auto rounded-md bg-muted/40 px-3 py-2 text-sm text-foreground/90"
      >
