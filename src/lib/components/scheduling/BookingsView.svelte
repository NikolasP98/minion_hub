<script lang="ts">
  ...
  // Posts to /api/sales/…, not /api/scheduling/…: `apiWriteCapability` derives
  // the required capability from the URL prefix, so the old scheduling path
  // authorized a sales write with `scheduling:edit`.
