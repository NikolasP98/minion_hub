<script lang="ts">
    ...
    const popoverApi = $derived(
        popover.connect(popoverService, normalizeProps),
    );
        ...
        modelQuery.trim()
            ? modelItems
                  .map((m) => ({
                      m,
                      score: Math.max(
                          fuzzyScore(modelQuery, m.id),
                          fuzzyScore(modelQuery, m.name),
                      ),
                  }))
                  .filter((x) => x.score > 0)
    ...
    const comboboxApi = $derived(
        combobox.connect(comboboxService, normalizeProps),
    );
    ...
    async function submit() {
        ...
        try {
            ...
            if (!newId) {
                throw new Error(
                    "Agent creation failed: server returned no agent ID",
                );
            }

            // Refresh agents list and confirm the new agent is actually present
            const listRes = (await sendRequest("agents.list", {})) as {
                agents?: Agent[];
            } | null;
            if (listRes?.agents) {
                ...
                if (
                    !listRes.agents.some(
                        (a) => (a as { id: string }).id === newId,
                    )
                ) {
                    throw new Error(
                        `Agent "${name.trim()}" was not found after creation — please refresh`,
                    );
                }
            }
...
>
    ...
    >
        ...
        >
            <div class="grid grid-cols-2 gap-2.5 mb-3">
                ...
                <div class="flex flex-col gap-1 col-span-full">
                    <div class="flex gap-2 items-center relative">
                        ...
                        <div {...popoverApi.getPositionerProps()}>
                            <div
                                class="absolute z-2000 bg-bg2 border border-border rounded-lg p-2 shadow-md w-55"
                                {...popoverApi.getContentProps()}
                            >
                                <div
                                    class="flex gap-0.5 mb-1.5 border-b border-border pb-1"
                                >
                                    {#each EMOJI_CATS as cat (cat.id)}
                                        <button
                                            class="bg-transparent border-none cursor-pointer text-base px-1 py-0.5 rounded-sm transition-all {activeCat ===
                                            cat.id
                                                ? 'opacity-100 bg-bg3'
                                                : 'opacity-50 hover:opacity-80 hover:bg-bg3'}"
                                            onclick={() => (activeCat = cat.id)}
                                            aria-label={cat.id}
                                            title={cat.id}>{cat.icon}</button
                                        >
                                    {/each}
                                </div>
                                <div class="grid grid-cols-6 gap-0.5">
                                    {#each EMOJI_CATS.find((c) => c.id === activeCat)?.emojis ?? [] as em (em)}
                                        <button
                                            class="bg-transparent border-none cursor-pointer text-lg p-0.75 rounded-sm leading-none transition-colors hover:bg-bg3"
                        ...
                        <input
                            id="agent-name"
                            class="flex-1 h-8.5 box-border bg-bg3 border border-border rounded-[5px] text-foreground px-3 py-0 font-mono text-[15px] font-medium outline-none transition-all placeholder:text-muted-foreground placeholder:font-normal focus:border-accent focus:shadow-[0_0_0_2px_color-mix(in_srgb,var(--color-accent)_20%,transparent)] disabled:opacity-50 disabled:cursor-not-allowed {nameError
                                ? 'border-destructive!'
                                : ''}"
                            type="text"
                            bind:value={name}
                    ...
                    {#if nameError}
                        <span class="text-[10px] text-destructive mt-px"
                            >{nameError}</span
                        >
                    {/if}
                </div>
                ...
                <div class="flex flex-col gap-1 col-span-full">
                    <div class="flex gap-2 mt-1">
                        <div class="flex flex-col gap-0.75 flex-1">
                            ...
                            <input
                                id="ws-user"
                                class="bg-bg3 border border-border rounded-[5px] text-foreground py-1 px-1.75 font-[inherit] text-xs outline-none transition-colors focus:border-accent disabled:opacity-50 disabled:cursor-not-allowed"
                                type="text"
                                bind:value={hostUser}
                                placeholder="minion"
                                disabled={saving}
                ...
                <div class="flex flex-col gap-1 col-span-full">
                    <div {...comboboxApi.getRootProps()}>
                        ...
                        >
                            <input
                                class="flex-1 min-w-0 bg-transparent border-none outline-none text-foreground py-1.25 px-2.25 font-[inherit] text-[13px] placeholder:text-muted-foreground disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        <div
                            class="z-2000 data-[state=closed]:hidden"
                            {...comboboxApi.getPositionerProps()}
                        >
                            <div
                                class="bg-bg2 border border-border rounded-md shadow-md overflow-hidden min-w-50 hidden:hidden"
                                {...comboboxApi.getContentProps()}
                            >
                                <ul
                                    class="list-none m-0 p-1 max-h-50 overflow-y-auto"
                                    {...comboboxApi.getListProps()}
                                >
                                    {#each filteredModels as item (item.id)}
                                        <li
                                            class="group flex items-center gap-1.5 py-1.25 px-2 rounded-sm text-xs cursor-pointer transition-colors data-highlighted:bg-bg3"
                                            {...comboboxApi.getItemProps({
                                                item,
                                            })}
                                        >
                                            <span
                                                class="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap group-data-selected:text-accent group-data-selected:font-semibold"
                                                {...comboboxApi.getItemTextProps(
                                                    { item },
                                                )}>{item.name}</span
                                            >
                                            ...
                                            {/if}
                                            <span
                                                class="text-muted-foreground text-[11px] font-mono shrink-0"
                                                >{item.id}</span
                                            >
                                        </li>
                                    {/each}
            ...
            {#if errorMsg}
                <div
                    class="bg-destructive/10 border border-destructive/30 rounded-md text-destructive text-xs py-2 px-3 mb-3"
                >
                    {errorMsg}
                </div>
            {/if}
            ...
            <div class="flex gap-2 justify-end">
                <button
                    type="button"
                    class="bg-transparent border border-border rounded-[5px] text-muted-foreground cursor-pointer font-[inherit] text-xs py-1.5 px-3 transition-colors hover:not-disabled:text-muted disabled:opacity-50 disabled:cursor-not-allowed"
                    onclick={close}
                    disabled={saving}>{m.agent_addCancel()}</button
                >
                <button
                    type="submit"
                    class="bg-accent border-none rounded-[5px] text-white cursor-pointer font-[inherit] text-xs font-semibold py-1.5 px-4 transition-[filter] hover:not-disabled:brightness-115 disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={saving}
                >
