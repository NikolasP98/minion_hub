...
<aside
    class="shrink-0 h-full flex flex-col border-r border-border bg-bg2/70 backdrop-blur transition-[width] duration-150 {collapsed
        ? 'w-12'
        : 'w-48'}"
>
    ...
    >
        {#if !collapsed}
            <span
                class="font-mono text-[10px] uppercase tracking-widest text-muted select-none"
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-2 flex flex-col gap-2">
        ...
        <div class="mt-1 pt-2 border-t border-border/50">
            {#if !collapsed}
                <span
                    class="font-mono text-[9px] uppercase tracking-widest text-muted-strong select-none"
                    >{m.workshop_elemLabel()}</span
                >
            {/if}
            <div
                class="mt-1.5 flex flex-wrap gap-1.5 {collapsed
                    ? 'justify-center'
                    : ''}"
            >
