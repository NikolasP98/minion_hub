<script lang="ts">
  ...
  let participants = $derived(
    (conversation?.participantInstanceIds ?? []).map((instanceId) => {
      ...
      const agent = instance ? gw.agents.find((a) => a.id === instance.agentId) : undefined;
      return {
        instanceId,
  ...
  let typeBadgeClass = $derived(
    conversation?.type === 'task' ? 'bg-accent/10 text-accent' : 'bg-bg3 text-muted-strong',
  );
...
<div
  class="chat-panel fixed right-0 top-0 h-full w-80 z-[var(--layer-modal)] flex flex-col bg-bg2/95 backdrop-blur border-l border-border"
...
>
  ...
  <div class="px-3 py-2 border-b border-border flex items-center justify-between gap-2 shrink-0">
    <div class="flex items-center gap-2 min-w-0">
      {#if conversation}
        <span class="text-xs font-mono uppercase px-1.5 py-0.5 rounded shrink-0 {typeBadgeClass}">
          {conversation.type}
        </span>
      {/if}
      <span class="text-xs text-foreground truncate">{participantNames}</span>
    </div>
    <Button
      variant="ghost"
      size="icon"
  ...
  <div class="flex-1 px-3 py-2 space-y-3 overflow-y-auto">
    {#if messages.length === 0}
      <div class="flex flex-col items-center justify-center h-full gap-2">
        ...
        {:else}
          <span class="text-xs text-muted">{m.workshop_noMessagesYet()}</span>
        {/if}
      </div>
    {:else}
        ...
        <div class="flex gap-2">
          ...
          <div class="min-w-0">
            <div class="flex items-baseline gap-1.5">
              <span class="text-xs font-mono text-muted">{msg.agentName}</span>
              <span class="text-xs text-muted-strong">{formatRelativeTime(msg.timestamp)}</span>
            </div>
            <p class="text-xs text-foreground leading-snug mt-0.5">{msg.text}</p>
