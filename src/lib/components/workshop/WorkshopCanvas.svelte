<script lang="ts">
  ...
  import { ELEMENT_WIDTH, ELEMENT_HEIGHT } from '$lib/workshop/renderer-adapter';
  import {
    startSimulation,
    stopSimulation,
    setBanterCallback,
  ...
  import {
    ...
    loadPixelOffice,
    autoSavePixelOffice,
  } from '$lib/state/workshop/pixel-office.svelte';
  ...
  async function initPixelOffice(): Promise<void> {
    ...
    try {
      ...
      console.info(
        `[Workshop Pixel] ${layout.cols}×${layout.rows} grid, zoom=${autoZoom}, ${Object.keys(workshopState.agents).length} agents`,
      );
   ...
   */
  function pixelWorldToScreen(worldX: number, worldY: number): { x: number; y: number } {
   ...
   */
  function worldToScreenAware(worldX: number, worldY: number): { x: number; y: number } {
   ...
   */
  function screenToWorldAware(screenX: number, screenY: number): { x: number; y: number } {
  ...
  function handlePointerMove(e: PointerEvent) {
    ...
    if (isPanning) {
      ...
      renderer.updateSpritePosition(draggedInstanceId, worldPos.x, worldPos.y);
      ...
      renderer.updateElementSpritePosition(draggedInstanceId, worldPos.x, worldPos.y);
  ...
  function handlePointerUp(e: PointerEvent) {
    ...
    if (isPixelPanning) {
      const wasDrag =
        Math.abs(e.clientX - pixelPanDragStartX) > 4 ||
    ...
    if (isDraggingAgent && draggedInstanceId) {
      ...
      const movedAgent = Math.abs(e.clientX - pointerDownX) + Math.abs(e.clientY - pointerDownY);
      if (movedAgent < 5) {
      ...
      const movedPan = Math.abs(e.clientX - pointerDownX) + Math.abs(e.clientY - pointerDownY);
  ...
  function handleWheel(e: WheelEvent) {
    ...
    if (workshopState.settings.viewMode === 'pixel' && pixelCanvas && pixelOffice) {
      ...
      const newZoom = e.deltaY > 0 ? Math.max(1, oldZoom - 1) : Math.min(8, oldZoom + 1);
  ...
  function cycleSelectedBehavior() {
    ...
    const next = cur === 'stationary' ? 'wander' : cur === 'wander' ? 'patrol' : 'stationary';
  ...
  function handleTaskPromptSubmit() {
    ...
    if (mode === 'assign') {
      ...
      const handle = startWorkshopConversation([instanceId, targetInstanceId], prompt);
      if (handle) {
        activeHandles.set(handle.conversationId, handle);
        sidebarOpen = true;
        selectedConversationId = handle.conversationId;
...
<div class="flex-1 relative overflow-hidden">
  ...
  {#if !conn.connected}
    <div
      class="absolute top-0 left-0 right-0 z-30 flex items-center justify-center gap-2 h-7 px-3 bg-destructive/15 border-b border-destructive/30 text-destructive text-xs font-mono pointer-events-none"
      role="status"
    >
  ...
  {#if showEmptyState}
    <div class="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
      ...
      >
        ...
        <span class="text-sm font-medium text-foreground">{m.workshop_emptyTitle()}</span>
        <span class="text-xs text-muted leading-relaxed">{m.workshop_emptyHint()}</span>
      </div>
    </div>
  {/if}
  ...
  <WorkshopConfigPanel
    ...
    onToggleChatRopes={() => {
      ...
      localStorage.setItem('workshop:showChatRopes', String(showChatRopes));
    }}
    {showRelationshipRopes}
    onToggleRelationshipRopes={() => {
      showRelationshipRopes = !showRelationshipRopes;
      localStorage.setItem('workshop:showRelationshipRopes', String(showRelationshipRopes));
    }}
    {debugMode}
    onToggleDebugMode={(v) => {
      debugMode = v;
  ...
  {#if activeOverlay}
    {#if activeOverlay.type === 'pinboard'}
      <PinboardOverlay elementId={activeOverlay.elementId} onClose={() => (activeOverlay = null)} />
    ...
    {:else if activeOverlay.type === 'inbox'}
      <InboxOverlay elementId={activeOverlay.elementId} onClose={() => (activeOverlay = null)} />
    {:else if activeOverlay.type === 'rulebook'}
      <RulebookOverlay elementId={activeOverlay.elementId} onClose={() => (activeOverlay = null)} />
    {:else if activeOverlay.type === 'portal'}
      <PortalOverlay elementId={activeOverlay.elementId} onClose={() => (activeOverlay = null)} />
    {/if}
  {/if}
