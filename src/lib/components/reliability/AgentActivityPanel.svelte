<script lang="ts">
  ...
  const outcomeColumns: DataColumn<ToolOutcomeRow>[] = [
    {
      key: 'tool',
      label: m.reliability_tool(),
      fill: true,
      cellClass: 'truncate max-w-[160px] text-foreground',
    },
    { key: 'ok', label: m.reliability_ok(), align: 'right', numeric: true, custom: true },
    {
      key: 'error',
      label: m.reliability_outcomeError(),
      align: 'right',
      numeric: true,
      custom: true,
    },
    {
      key: 'timeout',
      label: m.reliability_outcomeTimeout(),
      align: 'right',
      numeric: true,
      custom: true,
    },
    {
      key: 'authError',
      label: m.reliability_outcomeAuth(),
      align: 'right',
      numeric: true,
      custom: true,
    },
...
<div class="surface-2 rounded-lg overflow-hidden">
    ...
    {#snippet actions()}
      {#if usingActivity}
        <span class="text-[length:var(--font-size-telemetry)] text-muted-strong"
          >{m.reliability_fleetWideFullCoverage()}</span
        >
      {/if}
    {/snippet}
  </PanelHeader>
  ...
  {#if !hasAny}
    <div class="flex flex-col items-center justify-center gap-1 py-8 px-4 text-center">
      <span class="text-muted-foreground text-[length:var(--font-size-body)]"
        >{m.reliability_noActivitySignals()}</span
      >
  ...
  {:else}
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-border border-b border-border">
      <MetricCard
        label={m.reliability_memoriesSaved()}
        value={fmt(memory.created)}
        valueClass="text-[var(--color-purple)] tabular-nums"
      />
      <MetricCard
      ...
      />
      <MetricCard
        label={m.reliability_proactiveRuns()}
        value={pct(proactivity.ratio)}
        valueClass="text-[var(--color-brand)] tabular-nums"
      />
      <MetricCard
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-px bg-border">
      ...
      <div class="bg-bg2 p-3">
        <div class="flex items-center gap-1.5 pb-2">
          ...
          <span
            class="text-[length:var(--font-size-telemetry)] font-semibold uppercase tracking-widest text-muted-foreground"
            >{m.reliability_memoryKnowledgeGraph()}</span
          >
        </div>
        <div class="grid grid-cols-4 gap-2 text-center">
          <div class="rounded bg-bg3/40 py-2">
            ...
            <div
              class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
            >
              {m.reliability_created()}
            </div>
          </div>
          <div class="rounded bg-bg3/40 py-2">
            ...
            <div
              class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
            >
              {m.reliability_updated()}
            </div>
          </div>
          <div class="rounded bg-bg3/40 py-2">
            <div class="text-base font-bold text-muted-foreground tabular-nums">
              {fmt(memory.deleted)}
            </div>
            <div
              class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
            >
              {m.reliability_deleted()}
            </div>
          </div>
          <div class="rounded bg-bg3/40 py-2">
            <div class="text-base font-bold text-[var(--color-cyan)] tabular-nums">
              {fmt(memory.reads ?? 0)}
            </div>
            <div
              class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
        ...
        {#if memory.byType.length > 0}
          <div class="flex flex-wrap gap-1.5 mt-2.5">
            {#each memory.byType as t (t.key)}
              <span
                class="inline-flex items-center gap-1 rounded-full bg-[color-mix(in_srgb,var(--color-purple)_15%,transparent)] text-[var(--color-purple)] border border-[color-mix(in_srgb,var(--color-purple)_30%,transparent)] px-2 py-0.5 text-[length:var(--font-size-telemetry)]"
              >
        ...
        {#if memory.total > 0}
          <p class="text-[length:var(--font-size-telemetry)] text-muted-strong mt-2">
            {m.reliability_lastActivity({
              time: relTime(memory.lastTs),
              reads:
                (memory.reads ?? 0) > 0
                  ? ` · ${fmt(memory.reads ?? 0)} ${m.reliability_readCount()}`
                  : '',
            })}
          </p>
        ...
        {:else}
          <p class="text-[length:var(--font-size-telemetry)] text-warning mt-2">
            {m.reliability_noMemoryActivity()}
          </p>
        {/if}
      </div>
      ...
      <div class="bg-bg2 p-3">
        <div class="flex items-center gap-1.5 pb-2">
          ...
          <span
            class="text-[length:var(--font-size-telemetry)] font-semibold uppercase tracking-widest text-muted-foreground"
            >{m.reliability_heartbeatsLiveness()}</span
          >
        </div>
        {#if heartbeat.total > 0}
          <div class="grid grid-cols-3 gap-2 text-center">
            ...
            <div class="rounded bg-bg3/40 py-2">
              <div
                class="text-base font-bold {heartbeat.failed > 0
                  ? 'text-destructive'
                  : 'text-muted-foreground'} tabular-nums"
              >
                {fmt(heartbeat.failed)}
              </div>
              <div
                class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
              >
                {m.reliability_failed()}
              </div>
            </div>
            <div class="rounded bg-bg3/40 py-2">
              <div class="text-base font-bold text-muted-foreground tabular-nums">
                {fmt(heartbeat.skipped)}
              </div>
              <div
                class="text-[length:var(--font-size-telemetry)] uppercase tracking-wide text-muted-strong"
              >
                {m.reliability_skipped()}
              </div>
            </div>
          </div>
          <p class="text-[length:var(--font-size-telemetry)] text-muted-strong mt-2">
            {m.reliability_lastBeat({
              time: relTime(heartbeat.lastTs),
              status: heartbeat.lastStatus ? ` · ${heartbeat.lastStatus}` : '',
            })}
          </p>
        {:else}
          <div class="flex items-center justify-center h-[96px] text-center">
            <span class="text-[length:var(--font-size-label)] text-warning"
              >{m.reliability_noHeartbeats()}</span
            >
          </div>
        {/if}
      </div>
      ...
      <div class="bg-bg2 p-1">
        <div class="flex items-center gap-1.5 px-2 pt-2 pb-1">
          ...
          <span
            class="text-[length:var(--font-size-telemetry)] font-semibold uppercase tracking-widest text-muted-foreground"
            >{m.reliability_proactiveVsReactiveRuns()}</span
          >
        </div>
        ...
        {:else}
          <div
            class="flex items-center justify-center h-[180px] text-muted-strong text-[length:var(--font-size-label)]"
          >
            {m.reliability_noRunData()}
          </div>
        {/if}
      </div>
      ...
      <div class="bg-bg2 p-1">
        <div class="flex items-center gap-1.5 px-2 pt-2 pb-1">
          ...
          <span
            class="text-[length:var(--font-size-telemetry)] font-semibold uppercase tracking-widest text-muted-foreground"
            >{m.reliability_toolUsage({ count: fmt(tools.total) })}</span
          >
        </div>
    ...
    {#if toolOutcomes.total > 0}
      <div class="bg-bg2 p-3 border-t border-border">
        <div class="flex items-center gap-1.5 pb-2">
          ...
          <span
            class="text-[length:var(--font-size-telemetry)] font-semibold uppercase tracking-widest text-muted-foreground"
            >{m.reliability_toolOutcomes()}</span
          >
          <span
            class="ml-auto flex items-center gap-2 text-[length:var(--font-size-telemetry)] tabular-nums"
          >
            <span class="text-success">{fmt(toolOutcomes.ok)} {m.reliability_ok()}</span>
            {#if toolOutcomes.error > 0}<span class="text-warning"
                >{fmt(toolOutcomes.error)} {m.reliability_outcomeError()}</span
              >{/if}
            {#if toolOutcomes.timeout > 0}<span class="text-warning"
                >{fmt(toolOutcomes.timeout)} {m.reliability_outcomeTimeout()}</span
              >{/if}
            {#if toolOutcomes.authError > 0}<span class="text-destructive"
                >{fmt(toolOutcomes.authError)} {m.reliability_outcomeAuth()}</span
              >{/if}
          </span>
        </div>
          ...
          >
            {#snippet cell(t: ToolOutcomeRow, col: DataColumn<ToolOutcomeRow>)}
              ...
              {:else if col.key === 'error'}
                <span class="tabular-nums {t.error > 0 ? 'text-warning' : 'text-muted-strong'}"
                  >{fmt(t.error)}</span
                >
              {:else if col.key === 'timeout'}
                <span class="tabular-nums {t.timeout > 0 ? 'text-warning' : 'text-muted-strong'}"
                  >{fmt(t.timeout)}</span
                >
              {:else if col.key === 'authError'}
                <span
                  class="tabular-nums {t.authError > 0 ? 'text-destructive' : 'text-muted-strong'}"
                  >{fmt(t.authError)}</span
                >
              {/if}
            {/snippet}
          </DataTable>
