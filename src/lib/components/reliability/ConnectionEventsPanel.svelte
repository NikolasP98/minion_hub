<script lang="ts">
	...
	// Single consolidated events explorer ("Activity Log"). Driven by the page's
	// already date-ranged + chip-filtered `events`, plus the server-side `total`
	// and `byCategory` so the count badge and tab counts match the page header
	// (instead of an independent all-time fetch that drifted by ~3k). Replaces the
	// old standalone "Recent Incidents" table — table format, search, sort and
	// pagination now live here.
	let {
		events = [],
		total,
		byCategory = {},
	...
	// The badge/tab counts come from the server-side SQL summary (true totals for
	// the range), but the scatter + table operate on the loaded events (the
	// gateway caps the raw-event dump). Disclose the gap honestly instead of
	// silently plotting a subset under a larger headline number.
...
<div class="surface-2 w-full rounded-lg overflow-hidden">
	...
	<PanelHeader label={m.reliability_activityLog()} labelClass="shrink-0" class="flex-wrap gap-y-1.5">
		{#snippet icon()}
