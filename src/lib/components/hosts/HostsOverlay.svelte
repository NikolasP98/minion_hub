<script lang="ts">
  import { hostsState, addHost, updateHost, removeHost } from '$lib/state/features/hosts.svelte';
...
>
  ...
  >
    ...
    >
      <div class="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2.5">
      ...
      <div class="grid grid-cols-2 gap-2 mb-2.5">
        <div class="flex flex-col gap-1">
          ...
          <input
            id="host-name"
            class="bg-bg3 border border-border rounded-[var(--radius-sm)] text-foreground py-1.25 px-2.25 font-[inherit] text-xs outline-none transition-colors focus:border-accent"
            type="text"
            bind:value={formName}
            placeholder="protopi"
          />
        ...
        <div class="flex flex-col gap-1 col-span-2">
          <label for="host-url" class="text-[length:var(--font-size-caption)] text-muted-foreground"
            >{m.hosts_url()}</label
          >
          <input
            id="host-url"
      ...
      <div class="flex gap-2 justify-end">
        {#if editingId}
          <Button variant="secondary" size="sm" onclick={cancelEdit}>{m.hosts_cancel()}</Button>
        {/if}
