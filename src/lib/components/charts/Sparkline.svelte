<script lang="ts">
  ...
  let strokeColor = $derived(withAlpha(color, 0.8));
  let fillColorTop = $derived(withAlpha(color, 0.3));
  let fillColorBottom = $derived(withAlpha(color, 0));
</script>
...
<div class="w-full h-[20px]">
  ...
  >
    ...
    {#if paths.fillPath}
      <path d={paths.fillPath} fill="url(#{uid}-grad)" stroke="none" />
    {/if}
