...
>
  <div class="flex items-center gap-2 mb-1">
    <div class="w-6 h-6 rounded-md bg-[var(--color-surface-2)] flex items-center justify-center shrink-0">
      <FileText size={12} class="text-[var(--color-text-tertiary)]" />
    </div>
  ...
  {#if target}
    <div class="flex items-center gap-1.5">
      <code class="text-[length:var(--font-size-telemetry)] text-[var(--color-text-tertiary)] truncate max-w-[170px]">{target}</code>
      <span class="text-[length:var(--font-size-telemetry)] text-muted shrink-0">· {data.mode === 'append' ? m.flownode_append() : m.flownode_overwrite()}</span>
    </div>
  {:else}
    <p class="text-[length:var(--font-size-telemetry)] text-[var(--color-warning-fg)]/80">{m.flownode_doubleClickSetPath()}</p>
  {/if}
</div>
