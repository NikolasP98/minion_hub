<script lang="ts">
  ...
  const activeOrg = $derived(organizations.find((o) => o.id === activeOrgId) ?? organizations[0]);
  ...
  async function select(orgId: string) {
    ...
    try {
      ...
      toastError("Couldn't switch organization", err instanceof Error ? err.message : undefined);
    } finally {
      switchingTo = null;
    }
  }
...
{#if organizations.length > 0}
  <div class="relative w-full">
    <Button
      ...
      class="relative w-full flex items-center gap-1.5 h-6 px-1.5 rounded-[var(--radius-sm)] text-[length:var(--font-size-label)] font-medium transition-colors whitespace-nowrap select-none text-muted-foreground {canSwitch &&
      !switchingTo
