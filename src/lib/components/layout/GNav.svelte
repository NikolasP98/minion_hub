<script lang="ts">
    ...
    const chords: Chord[] = CHORD_DEFS.map((c): Chord | null => {
        ...
        if (item) {
            return { key: c.key, path: c.path, label: () => item.label, enabled: () => canViewPath(c.path) };
        }
        return null;
    }).filter((c): c is Chord => c !== null);

    for (const c of chords) {
        createHotkeySequence(['G', c.key], () => goto(c.path), () => ({
            enabled: c.enabled(),
            timeout: 1500,
        }));
    }
    ...
    const armed = $derived(registrations.sequences.some((s) => s.sequence[0] === 'G' && s.matchedStepCount > 0));
