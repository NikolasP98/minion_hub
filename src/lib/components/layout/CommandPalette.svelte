  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        />
        <kbd
          class="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-bg3 border border-border text-[10px] font-mono text-muted"
        >
          Esc
        </kbd>
      </div>
        ...
        {:else}
          ...
          {#each groups as group}
            ...
            {#each group.commands as cmd}
              ...
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div
                class="mx-2 px-3 py-2 rounded-lg flex items-center gap-3 cursor-pointer transition-colors duration-75 {isSelected
                  ? 'bg-accent/12 text-foreground'
                  : 'text-muted hover:bg-bg3 hover:text-foreground'}"
                data-cmd-index={currentIdx}
                onmouseenter={() => {
                  palette.selectedIndex = currentIdx;
                }}
