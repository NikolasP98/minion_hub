...
<header
  class="md:hidden shrink-0 relative z-50 bg-bg/95 backdrop-blur-md border-b border-[var(--hairline)] h-14"
>
  ...
  {#if mobileMenuOpen}
    ...
    <!-- Menu panel -->
    <div
      class="mobile-menu-panel absolute top-full left-0 right-0 z-50 bg-bg2/98 backdrop-blur-xl border-b border-border shadow-xl"
    >
      <!-- Scroll body: domains + sections -->
      <nav
        class="flex flex-col sm:flex-row sm:gap-4 max-h-[calc(70vh-3.5rem)] sm:max-h-[calc(75vh-3.5rem)] overflow-y-auto px-2 pt-2 pb-1"
      >
