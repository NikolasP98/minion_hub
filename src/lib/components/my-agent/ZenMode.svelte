...
<svelte:window
  ...
  onpointerdown={(e) => {
    if (polishMenuOpen && e.target instanceof Element && !e.target.closest('.zen-polish-wrap'))
      polishMenuOpen = false;
  }}
/>
    ...
    <div class="zen-header">
      {#if current.kind === 'note'}
        <div class="zen-polish-wrap">
          <Button
            type="button"
            class="zen-polish {polishMenuOpen ? 'on' : ''}"
      ...
      <Button
        type="button"
        class="zen-min"
        title={m.note_minimizeTitle()}
        aria-label={m.note_minimize()}
        onclick={onclose}
      >
        <Minimize2 size={18} />
      </Button>
      ...
      <div class="zen-title-row">
        {#if current.kind === 'note'}
          <NoteIconButton
            icon={current.icon ?? ''}
            size={26}
            onpick={(v) => setNoteIcon(current.id, v)}
          />
        {/if}
        <input
          class="zen-title"
