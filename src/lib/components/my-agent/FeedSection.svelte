...
<section class="feed-section" class:scrollable class:collapsed={isCollapsed}>
  <div class="header">
    {#if collapsible}
      <Button
        type="button"
        class="toggle {!collapsed ? 'open' : ''}"
...
<style>
  ...
  .feed-section.scrollable:not(.collapsed) .body {
    ...
    -webkit-mask-image: linear-gradient(to bottom, var(--color-bg) calc(100% - 14px), transparent);
    mask-image: linear-gradient(to bottom, var(--color-bg) calc(100% - 14px), transparent);
  }
</style>
