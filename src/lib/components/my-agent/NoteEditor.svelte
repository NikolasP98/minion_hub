<script lang="ts">
  ...
  createHotkey(
    'Alt+Space',
    () => editor?.commands.triggerAutofill(),
    () => ({
  ...
  function placeToolbarAtSelection() {
    ...
    const range =
      domSel && domSel.rangeCount > 0 && !domSel.isCollapsed ? domSel.getRangeAt(0) : null;
  ...
  function wordAt(
    pos: number,
  ): { from: number; to: number; word: string; sentence: string } | null {
  ...
  function onEditorContextMenu(e: MouseEvent) {
    ...
    if (clickInSelection) {
      ...
      toolbar = {
        left: Math.min(
          Math.max(e.clientX, TOOLBAR_HALF_W + 8),
          window.innerWidth - TOOLBAR_HALF_W - 8,
        ),
  ...
  const SLASH_ITEMS: { type: 'todo' | 'easel'; label: () => string; kw: string[] }[] = [
    {
      type: 'todo',
      label: () => m.note_slashMenuTodo(),
      kw: ['todo', 'task', 'tasks', 'check', 'list', 'checklist'],
    },
    {
      type: 'easel',
      label: () => m.note_slashMenuBoard(),
      kw: ['easel', 'board', 'moodboard', 'whiteboard', 'draw', 'image'],
    },
  ];
    ...
    untrack(() => {
      ...
      editor = new Editor({
        ...
        onUpdate({ editor: ed }) {
          const md =
            (
              ed.storage as unknown as Record<string, { getMarkdown?: () => string }>
...
{#if toolbar}
  ...
  >
    <div class="fmt-row">
      <Button
        type="button"
        class="fmt-btn {active.bold ? 'on' : ''}"
        title={m.note_formatBold()}
        aria-label={m.note_formatBold()}
        onclick={() => run((c) => c.toggleBold())}><Bold size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.italic ? 'on' : ''}"
        title={m.note_formatItalic()}
        aria-label={m.note_formatItalic()}
        onclick={() => run((c) => c.toggleItalic())}><Italic size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.strike ? 'on' : ''}"
        title={m.note_formatStrikethrough()}
        aria-label={m.note_formatStrikethrough()}
        onclick={() => run((c) => c.toggleStrike())}><Strikethrough size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.code ? 'on' : ''}"
        title={m.note_formatCode()}
        aria-label={m.note_formatCode()}
        onclick={() => run((c) => c.toggleCode())}><Code size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.highlight ? 'on' : ''}"
        title={m.note_formatHighlight()}
        aria-label={m.note_formatHighlight()}
        onclick={() => run((c) => c.toggleHighlight())}><Highlighter size={14} /></Button
      >
      <span class="fmt-sep"></span>
      <Button
        type="button"
        class="fmt-btn {active.h1 ? 'on' : ''}"
        title={m.note_formatHeading1()}
        aria-label={m.note_formatHeading1()}
        onclick={() => run((c) => c.toggleHeading({ level: 1 }))}><Heading1 size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.h2 ? 'on' : ''}"
        title={m.note_formatHeading2()}
        aria-label={m.note_formatHeading2()}
        onclick={() => run((c) => c.toggleHeading({ level: 2 }))}><Heading2 size={14} /></Button
      >
      <Button
        type="button"
        class="fmt-btn {active.bullet ? 'on' : ''}"
        title={m.note_formatBulletList()}
        aria-label={m.note_formatBulletList()}
        onclick={() => run((c) => c.toggleBulletList())}><List size={14} /></Button
      >
      <span class="fmt-sep"></span>
      <Button
        type="button"
        class="fmt-btn"
        title={m.note_formatClear()}
        aria-label={m.note_formatClear()}
  ...
  >
    {#each slashMatches as item, idx (item.type)}
      <Button
        type="button"
        role="menuitem"
        class="slash-item {idx === slashIndex ? 'active' : ''}"
...
<svelte:window
  ...
  onscroll={() => {
    if (toolbar && !pinned) syncToolbar();
    else if (toolbar) closeToolbar();
  }}
  onpointerdown={(e) => {
    ...
    if (toolbar && e.target instanceof Node && !(e.target as Element).closest?.('.fmt-toolbar'))
      closeToolbar();
  }}
/>
...
<style>
  ...
  .fmt-spell {
    ...
    padding: var(--space-1) var(--space-1) var(--space-0-5);
