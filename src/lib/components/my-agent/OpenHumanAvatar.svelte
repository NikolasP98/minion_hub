...
<svg viewBox="0 0 1000 1000" class="avatar-svg" aria-hidden="true">
  <defs>
    <radialGradient id="oh-body" cx="0.36" cy="0.3" r="0.95">
      <stop offset="0%" stop-color="var(--color-destructive)" />
      <stop offset="45%" stop-color="var(--color-warning)" />
  ...
  <ellipse
    ...
    rx="210"
    ry="34"
    fill="color-mix(in srgb, var(--color-bg) 28%, transparent)"
  />

  <!-- legs behind body (stay grounded) -->
  <path d={LEFT_LEG} fill="var(--color-warning)" />
  <path d={RIGHT_LEG} fill="var(--color-warning)" />

  <!-- waving arm -->
  <g bind:this={armEl}>
    <path d={ARM_PATH} fill="var(--color-warning)" stroke="var(--color-warning)" stroke-width="6" />
  </g>

  <!-- body + face breathe together -->
  <g bind:this={bodyG}>
    <path d={BODY_PATH} fill="url(#oh-body)" stroke="var(--color-warning)" stroke-width="6" />
    <!-- eyes (dots) -->
    <circle cx="418" cy="500" r="40" fill="var(--color-bg2)" />
    <circle cx="622" cy="500" r="40" fill="var(--color-bg2)" />
    <circle cx="432" cy="486" r="13" fill="var(--color-foreground)" opacity="0.9" />
    <circle cx="636" cy="486" r="13" fill="var(--color-foreground)" opacity="0.9" />
  ...
  <g bind:this={dotsEl} style="opacity: 0">
    <circle cx="700" cy="300" r="16" fill="var(--color-warning)" />
    <circle cx="755" cy="265" r="22" fill="var(--color-warning)" />
    <circle cx="820" cy="220" r="30" fill="var(--color-warning)" />
  </g>
</svg>
