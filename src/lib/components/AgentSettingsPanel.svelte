<script lang="ts">
  ...
  const resolved = $derived(
    configState.loaded
      ? buildGroupedFields(configState.current, configState.schema, agentId)
      : null
  );
  ...
  const hasContent = $derived(visibleGroups.length > 0 || bindingEntries.length > 0);
...
>
  <!-- Panel -->
  <div
    class="absolute top-0 right-0 h-full w-[420px] max-w-full bg-bg2 border-l border-border flex flex-col shadow-2xl"
    role="dialog"
    aria-label="Agent settings for {agentId}"
  >
    <!-- Header -->
    <div class="shrink-0 flex items-center gap-3 px-4 py-3.5 border-b border-border">
    ...
    {:else if configState.loadError}
      <div class="flex-1 flex items-center justify-center px-4">
        <div class="text-center">
          <p class="text-destructive text-xs mb-2">Failed to load config</p>
          <p class="text-muted-foreground text-[11px] mb-3">
            {configState.loadError}
          </p>
          <button
            type="button"
            class="bg-accent border-none rounded-[5px] text-white cursor-pointer font-[inherit] text-xs font-semibold py-[5px] px-3"
            onclick={() => loadConfig()}
          >
            Retry
          </button>
    ...
    {:else if !hasContent}
      <div class="flex-1 flex items-center justify-center px-4">
        <p class="text-muted-foreground text-xs text-center">
          No configuration loaded for <strong class="text-foreground">{agentId}</strong>.
        </p>
      </div>
    {:else}
      <div class="flex-1 overflow-y-auto">
        ...
        {#if isListStructure}
          <div class="mx-4 mt-3 px-3 py-2 bg-accent/8 border border-accent/20 rounded-lg">
            <p class="text-[11px] text-muted-foreground leading-relaxed m-0">
              Fields inherit from <strong class="text-foreground">agents.defaults</strong> unless overridden here.
            </p>
          </div>
        {/if}
              ...
              >
                ...
                <span class="flex flex-col items-start gap-0.5 min-w-0">
                  <span class="flex items-center gap-2">
                    <span class="text-[11px] font-semibold text-foreground uppercase tracking-wider"
                      >{group.label}</span
                    >
                    ...
                    <div class="relative group/field">
                      ...
                      {#if field.isOverridden && field.defaultValue !== undefined}
                        <button
                          ...
                          title="Reset to default: {JSON.stringify(field.defaultValue)}"
                        >
                          reset
                        </button>
                      {/if}
        ...
        {#if bindingEntries.length > 0}
          <div class="border-t border-border mx-4 pt-3 pb-4 space-y-3">
            <h3
            ...
            {#each bindingEntries as [bindKey, bindVal] (bindKey)}
              ...
              {@const bindSchema =
                ...
                (typeof bindingsSchema?.additionalProperties === 'object'
                  ? bindingsSchema.additionalProperties
                  : { type: 'string', title: bindKey })}
              {@const bindHint = configState.uiHints[bindPath] ?? {}}
              <ConfigField
                path={bindPath}
                schema={bindSchema}
                hint={bindHint}
      ...
      {#if isDirty.value || configState.saving || configState.saveError}
        ...
        >
          {#if configState.saveError}
            <span class="text-destructive text-[11px] flex-1 truncate">
              {configState.saveError}
            </span>
          {:else if configState.lastSavedAt && !isDirty.value}
            <span class="text-success text-[11px] flex-1">Saved</span>
          {:else}
            <span class="flex-1"></span>
          {/if}
          ...
          <button
            type="button"
            class="bg-transparent border border-border rounded-[5px] text-muted-foreground cursor-pointer font-[inherit] text-xs py-[5px] px-3 transition-colors hover:text-foreground disabled:opacity-40 disabled:cursor-default"
          ...
          <button
            type="button"
            class="bg-accent border-none rounded-[5px] text-white cursor-pointer font-[inherit] text-xs font-semibold py-[5px] px-3 transition-[filter] hover:brightness-115 disabled:opacity-40 disabled:cursor-default flex items-center gap-2"
            disabled={!isDirty.value || configState.saving}
            onclick={() => save()}
          >
