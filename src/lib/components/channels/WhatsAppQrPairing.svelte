...
<div class="space-y-3">
  ...
  {:else if pairingStatus === 'waiting' && qrData}
    <div
      class="flex flex-col items-center gap-3 p-4 bg-[var(--color-qr-canvas,white)] rounded-lg w-fit mx-auto"
      {...api.getRootProps()}
    >
      <svg class="h-48 w-48" {...api.getFrameProps()}>
        <path {...api.getPatternProps()} />
      </svg>
      <p class="text-xs text-[var(--color-qr-ink-muted,var(--color-canvas))]">
        {m.channel_scanWithWhatsApp()}
      </p>
    </div>
  ...
  {:else if pairingStatus === 'error'}
    ...
    <Button variant="outline" size="sm" type="button" onclick={startPairing}>
      {m.common_retry()}
    </Button>
  {/if}
</div>
