<script lang="ts">
  ...
  } from '$lib/utils/channel-display-state';
  import {
    CircleCheck,
    CircleX,
    AlertTriangle,
  ...
  > = {
    ...
    'pending-config': {
      ...
      text: 'text-warning',
      dot: 'bg-warning',
      label: m.channelState_pendingConfig,
      pulse: false,
      Icon: AlertTriangle,
    },
    'not-linked': {
      ...
      text: 'text-muted-foreground',
      dot: 'bg-muted-foreground',
      label: m.channelState_notLinked,
      pulse: false,
      Icon: Link2Off,
    },
    'identity-mismatch': {
      ...
      text: 'text-warning',
      dot: 'bg-warning',
      label: m.channelState_identityMismatch,
      pulse: false,
      Icon: UserX,
    },
    starting: {
    ...
    'sync-stalled': {
      ...
      text: 'text-warning',
      dot: 'bg-warning',
      label: m.channelState_syncStalled,
      pulse: false,
      Icon: PauseCircle,
    },
    error: {
  ...
  {#snippet content()}
    <div class="min-w-[200px]">
      ...
      {#if state === 'identity-mismatch'}
        <div
          class="mt-2 pt-2 border-t border-border/60 text-xs text-warning break-words max-w-[260px]"
        >
          <div>
            Linked: <span class="font-medium tabular-nums">{linkedNumber ?? 'unknown'}</span>
          </div>
          {#if expectedNumber}
      ...
      {:else if state === 'not-linked'}
        <p
          class="mt-2 pt-2 border-t border-border/60 text-xs text-muted-foreground break-words max-w-[260px]"
        >
          No device linked. Connect and scan the QR to pair a phone.
        </p>
      {:else if channel.gwLastError}
        <p
          class="mt-2 pt-2 border-t border-border/60 text-xs text-destructive break-words max-w-[260px]"
        >
          {channel.gwLastError}
        </p>
      {/if}
