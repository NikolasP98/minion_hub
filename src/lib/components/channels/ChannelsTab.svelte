<script lang="ts">
  ...
  import { channelState, fetchChannels, updateChannel, deleteChannel } from '$lib/state/channels';
  ...
  const liveChannels = $derived.by((): Channel[] => {
    ...
    for (const [channelType, accounts] of Object.entries(ca)) {
      const type = (
        CHANNEL_TYPE_LABELS[channelType as ChannelType] ? channelType : 'discord'
      ) as ChannelType;
      for (const acctRaw of accounts) {
        ...
        if (acct.accountId === 'default' && !acct.configured && !acct.connected && !acct.running) {
          continue;
        }
        ...
        if (acct.identityMismatch && acct.expectedIdentity)
          meta.expectedPhone = acct.expectedIdentity;
  ...
  const mergedChannels = $derived.by((): Channel[] => {
    ...
    const gwAccountOf = (c: Channel) =>
      (c.id.startsWith('gw:') ? c.id.split(':')[2] : c.accountId) ?? null;
    ...
    const extras = gatewayChannels.filter((gwCh) => {
      const acct = gwAccountOf(gwCh);
      return (
        !hubKeys.has(`${gwCh.type}:${gwCh.label.toLowerCase()}`) &&
  ...
  async function fetchHeartbeatChannels(sid: string) {
    try {
      ...
      const result: Channel[] = [];
      for (const [channelType, accounts] of Object.entries(
        data.channelAccounts as Record<
          string,
          Record<
            string,
            {
...
<div class="space-y-4">
  ...
  {:else}
    ...
    {#each ['telegram', 'whatsapp', 'discord'] as t (t)}
      ...
      {@const configChannels = configState.original?.channels as
