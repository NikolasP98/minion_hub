<script lang="ts">
  ...
  import { findHistorySync, findHubSync } from '$lib/state/gateway';
  ...
  const delivery = $derived(
    findHubSync(channelType, verified?.kind === 'whatsapp' ? verified.phone : null),
  );
...
<div class="bg-card border border-border rounded-lg p-5 space-y-4">
      ...
      <div class="flex items-start gap-3 p-3 bg-success/10 border border-success/30 rounded-md">
        ...
        <div class="flex-1 min-w-0 text-xs space-y-0.5">
          ...
          {:else if verified.kind === 'discord'}
            <p class="text-muted-foreground">
              {verified.username} · id {verified.id}{#if verified.appName}
                · app
                {verified.appName}{/if}
            </p>
          {/if}
  ...
  {:else if step === 'sync'}
    ...
    <ChannelSyncStatus
      ...
      {delivery}
      {serverId}
      accountId={verified?.kind === 'whatsapp' ? verified.phone : null}
    />
    <Button type="button" variant="primary" onclick={onclose}>
