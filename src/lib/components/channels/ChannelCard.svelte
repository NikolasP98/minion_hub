<script lang="ts">
  ...
  let allowlistText = $state((channel.allowFrom ?? []).filter((s) => s !== '*').join('\n'));
  ...
  function handleSelectMode(mode: 'receive' | 'allowlist' | 'open') {
    ...
    if (mode === 'receive') {
      ...
      void saveBehavior({ replies: 'bound', allowFrom: ['*'] }, m.channel_behaviorSavedOpen());
    }
    ...
    Boolean(
      ...
      (
        configState.current?.channels as
          Record<string, { accounts?: Record<string, { markOnline?: boolean }> }> | undefined
      )?.whatsapp?.accounts?.[gwAccountId]?.markOnline,
    ),
  );
  ...
  async function handleToggleMarkOnline() {
    ...
    try {
      ...
      const patch = {
        channels: { whatsapp: { accounts: { [gwAccountId]: { markOnline: next } } } },
      };
...
>
  ...
  <div class="flex items-center gap-3 px-4 py-3 group">
    ...
    >
      <span class="flex min-w-0 flex-1 flex-col items-start">
        <span class="flex items-baseline gap-2">
          ...
          {#if channel.credentialsMeta?.username}
            <span class="text-xs text-muted-strong truncate"
              >@{channel.credentialsMeta.username}</span
            >
          {/if}
          {#if channel.credentialsMeta?.phone}
            <span
              class="text-xs truncate tabular-nums {channel.gwIdentityMismatch
                ? 'text-warning font-medium'
                : 'text-muted-strong'}"
              title={channel.gwIdentityMismatch
                ? `Linked to ${channel.credentialsMeta.phone}${channel.credentialsMeta.expectedPhone ? ` — expected ${channel.credentialsMeta.expectedPhone}` : ''}`
                : channel.credentialsMeta.phone}
            >
      ...
      {#if channel.replies}
        <span
          class="shrink-0 text-xs font-medium px-1.5 py-0.5 rounded-full {channel.replies ===
          'bound'
            ? 'bg-accent/10 text-accent'
            : 'bg-muted text-muted-foreground'}"
          title={channel.replies === 'bound'
            ? m.channel_modeBoundHint()
            : m.channel_modeReceiveOnlyHint()}
        >
  ...
  {#if isSyncActive(channel.historySync, channel.hubSync)}
    <div class="px-4 pb-3 -mt-1">
      <ChannelSyncStatus
        sync={channel.historySync}
        delivery={channel.hubSync}
        {serverId}
      ...
      <div class="px-4 pb-4 space-y-4 border-t border-border/50 pt-3">
        ...
        <dl class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
          ...
          <dd class="text-foreground truncate">
            {gwAccountId ?? channel.accountId ?? channel.label}
          </dd>
        ...
        {#if channel.gwLastError}
          <details class="text-xs">
            ...
            <pre
              class="mt-1 p-2 bg-bg3 rounded text-destructive overflow-x-auto whitespace-pre-wrap break-words">{channel.gwLastError}</pre>
          </details>
        {/if}
          ...
          <div>
            ...
            {#if behaviorMode === 'allowlist'}
              <div class="mt-2 space-y-1.5">
                <label class="text-xs text-muted-foreground" for={`allowlist-${channel.id}`}
                  >{m.channel_behaviorAllowlistLabel()}</label
                >
                <textarea
          ...
          <div>
            ...
            {:else}
              <div class="flex flex-wrap items-center gap-1.5">
                {#each directBindings as b, i (i)}
                  <span
                    class="text-xs font-medium px-1.5 py-0.5 rounded-full bg-accent/10 text-accent"
                  >
                    {b.agentId ?? m.channel_routingNoAgent()}{b.match.accountId === '*'
                      ? ` (${m.channel_routingCatchAll()})`
                      : ''}
                  </span>
                {/each}
              </div>
            ...
            {#if peerBindingCount > 0}
              <p class="text-xs text-muted-foreground mt-1">
                {m.channel_routingPeerRules({ count: peerBindingCount })}
              </p>
            {/if}
            <a
              href="/settings?s=agents"
              class="text-xs text-accent hover:underline mt-1 inline-block"
              onclick={(e) => e.stopPropagation()}>{m.channel_routingEdit()}</a
            >
          </div>
        {/if}
