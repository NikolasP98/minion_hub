<script lang="ts">
    ...
    $effect(() => {
        ...
        void activeBody; void activeId; void isDirty;
        void disabledCount; void sectionCount;
        if (!agentId) return;
        schedulePreview();
    });
...
<div class="flex flex-col h-full min-h-0 overflow-hidden">
    ...
    {#if !promptSections.agentId}
        <EmptyState />
    {:else}
        <div {...hApi.getRootProps()} class="flex-1 min-h-0 flex overflow-hidden">
            ...
            <div
                ...
                class="overflow-hidden border-l border-border min-w-0"
            >
                <div {...vApi.getRootProps()} class="flex flex-col h-full">
                    ...
                    <div
                        ...
                        class="h-1 bg-border hover:bg-accent cursor-row-resize"
                    ></div>
                    <div
                        {...vApi.getPanelProps({ id: "detail" })}
                        class="overflow-hidden border-t border-border min-h-0"
                    >
                        <SelectionDetail />
                    </div>
                </div>
