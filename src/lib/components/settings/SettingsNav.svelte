<script lang="ts">
  ...
  const HUB_TABS: HubTab[] = [
    {
      id: 'organizations',
      label: m.settings_nav_organizations(),
      icon: 'Building2',
    ...
    {
      id: 'workflows',
      label: m.settings_nav_workflows(),
      icon: 'Workflow',
  ...
  const visibleHubTabs = $derived(
    (isAdmin.value ? HUB_TABS : HUB_TABS.filter((t) => !t.adminOnly)).filter((t) =>
      canViewPath(t.href),
    ),
  );
  const visibleTeamTabs = $derived(
    (isAdmin.value ? TEAM_TABS : []).filter((t) => canViewPath(t.href)),
  );
  ...
  const groups = $derived.by<SectionNavGroup[]>(() => {
    ...
    if (visibleHubTabs.length)
      out.push({
        id: 'hub',
        label: m.settings_nav_group_hub(),
        items: visibleHubTabs.map(hubItem),
      });
    if (visibleTeamTabs.length)
      out.push({
        id: 'team',
        label: m.settings_nav_group_team(),
        items: visibleTeamTabs.map(hubItem),
      });
    return out;
  });
