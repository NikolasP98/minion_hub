...
let s = $state<SyncState>({ active: false, status: null, total: null, processed: 0, error: null, stalled: false });
...
export const financeSync = {
  ...
  async start(provider = 'susii') {
    ...
    try {
      ...
      s.active = true; s.status = 'running'; s.stalled = false;
