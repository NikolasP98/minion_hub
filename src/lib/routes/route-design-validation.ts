  ...
  if (!sameValues(immersive, IMMERSIVE_SCREEN_PATTERNS)) {
    output.push(
      issue(
        ...
        'Immersive family must cover Home, Autonomous, Workshop and Terminal routes.',
      ),
    );
  }
...
export function validateNavReferences(
  ...
  for (const reference of references) {
    const path = navPath(reference.href);
    if (path === '/') continue; // hooks-level landing redirect, intentionally outside the screen manifest.
    if (!known.has(path) && !knownShapes.has(routeShape(path))) {
      output.push(
        issue(
          'nav.unknown-href',
