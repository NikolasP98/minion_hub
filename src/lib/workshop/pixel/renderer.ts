...
import { getBubbleSprite, getCharacterSprites } from './sprite-data';
import type {
  Character,
  FloorColor,
  FurnitureInstance,
...
import { getWallInstances, hasWallSprites, wallColorToHex } from './wall-tiles';
import { getCharacterSprite, getDragState } from './characters';
import { renderMatrixEffect } from './matrix-effect';
import { getInstanceForCharId } from './gateway-pixel-bridge';
...
export function renderScene(
  ...
  for (const f of furniture) {
    ...
    const isOnState = f.isOnState === true;
    if (f.mirrored) {
      drawables.push({
        zY: f.zY,
        draw: (c) => {
          c.save();
          if (isOnState) {
            c.shadowColor = 'rgba(0, 255, 120, 0.4)';
            c.shadowBlur = 6;
          }
  ...
  // Drag state: dragged character renders at cursor position with 0.85 alpha
  const dragState = getDragState();
  const draggedCharId = dragState.charId;

  // Characters
  for (const ch of characters) {
    const sprites = getCharacterSprites(ch.palette, ch.hueShift);
    const spriteData = getCharacterSprite(ch, sprites);
    if (!spriteData || spriteData.length === 0) continue; // skip empty sprites
    const cached = getCachedSprite(spriteData, zoom);

    // If being dragged, render at drag position instead
    if (ch.id === draggedCharId) {
      const dragDrawX = Math.round(offsetX + dragState.worldX * zoom - cached.width / 2);
      const dragDrawY = Math.round(
        offsetY + (dragState.worldY + CHARACTER_DRAG_OFFSET_Y) * zoom - cached.height,
      );
      const dragCached = cached;
      // Render at very high zY so dragged character is always on top
      drawables.push({
        zY: 99999,
        draw: (c) => {
          c.save();
          c.globalAlpha = 0.85;
          c.drawImage(dragCached, dragDrawX, dragDrawY);
          c.restore();
        },
      });
      continue;
    ...
    drawables.push({
      ...
      draw: (c) => {
        if (isSubagent) {
          ...
          const cx = charDrawX + charCached.width / 2;
          const cy = charDrawY + charCached.height / 2;
          c.translate(cx, cy);
          c.scale(0.75, 0.75);
          c.translate(-cx, -cy);
          c.drawImage(charCached, charDrawX, charDrawY);
          c.restore();
        } else {
          c.drawImage(charCached, charDrawX, charDrawY);
        }
 ...
 *  IMPROVEMENT: setLineDash is called once before the loop instead of per-tile. */
export function renderGhostBorder(
  ...
  // Set line dash once before iterating tiles (moved out of loop)
  ctx.lineWidth = 1;
  ctx.setLineDash(VOID_TILE_DASH_PATTERN);
...
export function renderBubbles(
  ...
  for (const ch of characters) {
    ...
    // Position: centered above the character's head, follow sitting offset
    const sittingOff = ch.state === CharacterState.TYPE ? BUBBLE_SITTING_OFFSET_PX : 0;
    // bubbleCenterX: horizontal center of bubble
    const bubbleCenterX = offsetX + ch.x * zoom;
    // bubbleCenterY: vertical center of bubble (above character head)
    const bubbleCenterY = offsetY + (ch.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom;

    // Permission bubble: animated amber dots (D-19)
    if (ch.bubbleType === 'permission') {
      ctx.save();
      // Draw amber background pill
      const pillW = 16 * zoom;
      const pillH = 8 * zoom;
      const pillX = bubbleCenterX - pillW / 2;
      const pillY = bubbleCenterY - pillH / 2;
      ...
      const dotsStartX = bubbleCenterX - dotSpacing;

      for (let i = 0; i < 3; i++) {
        ctx.fillStyle = i === dotPhase ? 'rgba(255, 255, 255, 1.0)' : 'rgba(255, 255, 255, 0.3)';
        ctx.beginPath();
        ctx.arc(dotsStartX + i * dotSpacing, bubbleCenterY, dotRadius, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
      continue;
    ...
    // Waiting bubble: sprite-based with fade
    const sprite = getBubbleSprite(ch.bubbleType);
    if (!sprite) continue;

    // Compute opacity: waiting = fade in last 0.5s
    let alpha = 1.0;
    ...
    const cached = getCachedSprite(sprite, zoom);
    const bubbleX = Math.round(bubbleCenterX - cached.width / 2);
    const bubbleY = Math.round(bubbleCenterY - cached.height - 1 * zoom);
...
/**
 * Render agent name labels above all characters (D-12).
 * Always visible, drawn on top of everything else.
 */
  ...
  for (const ch of characters) {
    ...
    if (ch.isSubagent) {
      ...
      const parentInstanceId =
        ch.parentAgentId !== null ? getInstanceForCharId(ch.parentAgentId) : undefined;
      ...
      const agentName =
        gw.agents.find((a) => a.id === agentInst.agentId)?.name ?? agentInst.agentId;
...
/**
 * Render seat highlight overlays during drag-to-reassign.
 * Available seats show green, occupied seats show red.
 */
function renderDragOverlays(
  ...
  for (const seat of seats.values()) {
    const x = offsetX + seat.seatCol * s;
    const y = offsetY + seat.seatRow * s;
    ctx.fillStyle = seat.assigned ? SEAT_BUSY_COLOR : SEAT_AVAILABLE_COLOR;
    ctx.fillRect(x, y, s, s);
  }
}
...
export function renderFrame(
  ...
  // Drag overlays: seat highlights during drag-to-reassign
  const activeDragState = getDragState();
  if (activeDragState.charId !== null && selection) {
    renderDragOverlays(ctx, selection.seats, offsetX, offsetY, zoom);
  }

  // Speech bubbles (always on top of characters)
  renderBubbles(ctx, characters, offsetX, offsetY, zoom);

  // Name labels (always on top of everything, including bubbles)
