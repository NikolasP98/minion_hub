...
import { DEFAULT_COLS, DEFAULT_ROWS, Direction, MAX_COLS, TILE_SIZE, TileType } from './types';
import { getCatalogEntry, getOrientationInGroup } from './furniture-catalog';
...
export function layoutToFurnitureInstances(furniture: PlacedFurniture[]): FurnitureInstance[] {
	...
	for (const item of furniture) {
		...
		const isOnState = item.type.endsWith('_on');
		instances.push({ sprite, x, y, zY, ...(mirrored ? { mirrored: true } : {}), ...(isOnState ? { isOnState: true } : {}) });
	}
	return instances;
}
...
export function getBlockedTiles(
	...
	for (const item of furniture) {
		...
		for (let dr = 0; dr < entry.footprintH; dr++) {
			...
			for (let dc = 0; dc < entry.footprintW; dc++) {
				const key = (item.col + dc) + (item.row + dr) * MAX_COLS;
...
export function getPlacementBlockedTiles(
	...
	for (const item of furniture) {
		...
		for (let dr = 0; dr < entry.footprintH; dr++) {
			...
			for (let dc = 0; dc < entry.footprintW; dc++) {
				tiles.add((item.col + dc) + (item.row + dr) * MAX_COLS);
			}
		}
	}
	return tiles;
