 ...
 */
function extractTilesFromSheet(imageData: ImageData, tileW: number, tileH: number): SpriteData[] {
...
async function loadFurnitureAssets(basePath: string): Promise<void> {
  ...
  try {
    ...
    if (Object.keys(sprites).length > 0) {
      ...
      console.info(
        `[Workshop Pixel] Loaded ${Object.keys(sprites).length}/${catalog.length} furniture sprites`,
      );
    }
  } catch {
