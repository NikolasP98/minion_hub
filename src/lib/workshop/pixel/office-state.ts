...
import { getAnimationFrames, getCatalogEntry, getOnStateType } from './furniture-catalog';
import {
	createDefaultLayout,
	getBlockedTiles,
...
import { findPath, getWalkableTiles, isWalkable } from './tile-map';
import type {
	Character,
	FurnitureInstance,
	OfficeLayout,
...
import { CharacterState, Direction, MATRIX_EFFECT_DURATION_SEC, MAX_COLS, TILE_SIZE } from './types';
...
export class OfficeState {
	...
	/**
	 * Cached array of characters, rebuilt only on add/remove.
	 * Avoids Array.from() allocation every frame in getCharacters().
	 */
	private _characterArray: Character[] = [];
	private _characterArrayDirty = true;
	...
	/** Mark the cached character array as needing rebuild */
	private invalidateCharacterArray(): void {
		this._characterArrayDirty = true;
	}
	...
	private ownSeatKey(ch: Character): number | null {
		if (!ch.seatId) return null;
		const seat = this.seats.get(ch.seatId);
		if (!seat) return null;
		return seat.seatCol + seat.seatRow * MAX_COLS;
	}

	/** Temporarily unblock a character's own seat in blockedSet, run fn, then re-block */
	private withOwnSeatUnblocked<T>(ch: Character, blockedSet: Set<number>, fn: () => T): T {
		const key = this.ownSeatKey(ch);
		if (key) blockedSet.delete(key);
		const result = fn();
	...
	reassignSeat(agentId: number, seatId: string): void {
		...
		const path = this.withOwnSeatUnblocked(ch, this.blockedTiles, () =>
	...
	walkToTile(agentId: number, col: number, row: number): boolean {
		...
		if (!isWalkable(col, row, this.tileMap, this.blockedTiles)) {
			...
			const key = this.ownSeatKey(ch);
			if (key === null || key !== col + row * MAX_COLS) return false;
		}
		const path = this.withOwnSeatUnblocked(ch, this.blockedTiles, () =>
	...
	update(dt: number): void {
		...
		for (const ch of this.characters.values()) {
			...
			// Build per-character dynamic blocked set: static tiles + all other characters' positions
			const dynamicBlocked = new Set(this.blockedTiles);
			for (const other of this.characters.values()) {
				if (other.id === ch.id) continue;
				// Don't block tiles of despawning characters (they are already leaving)
	...
	/**
	 * Get all characters as an array. Uses a cached array that is only
	 * rebuilt when characters are added or removed, avoiding Array.from()
	 * allocation on every frame.
	 */
	getCharacters(): Character[] {
		if (this._characterArrayDirty) {
			this._characterArray = Array.from(this.characters.values());
			this._characterArrayDirty = false;
		}
