...
export function isJobStale(
  job: Pick<FinSyncJob, 'status' | 'heartbeatAt'>,
  now = Date.now(),
): boolean {
    ...
    const [row] = await tx
      ...
      .where(
        and(
          ...
          eq(finSyncJobs.provider, provider),
          inArray(finSyncJobs.status, ACTIVE),
        ),
      )
      .orderBy(desc(finSyncJobs.createdAt))
      .limit(1);
    return row ?? null;
  });
}
...
export async function claimJob(ctx: CoreCtx, jobId: string): Promise<boolean> {
  return withOrgCore(ctx, async (tx) => {
    const rows = await tx
      ...
      .set({
        status: 'running',
        startedAt: sql`coalesce(${finSyncJobs.startedAt}, now())`,
        heartbeatAt: sql`now()`,
        updatedAt: sql`now()`,
      })
      .where(
        and(
          ...
          eq(finSyncJobs.orgId, ctx.tenantId),
          or(
            eq(finSyncJobs.status, 'queued'),
            and(eq(finSyncJobs.status, 'running'), lt(finSyncJobs.heartbeatAt, staleClause)),
          ),
        ),
      )
...
export async function isCancelRequested(ctx: CoreCtx, jobId: string): Promise<boolean> {
  return withOrgCore(ctx, async (tx) => {
    const [row] = await tx
      .select({ cancelRequested: finSyncJobs.cancelRequested })
      .from(finSyncJobs)
      .where(and(eq(finSyncJobs.id, jobId), eq(finSyncJobs.orgId, ctx.tenantId)))
      .limit(1);
...
export async function findResumableJobs(
  limit = 3,
): Promise<Array<{ jobId: string; orgId: string; provider: string }>> {
  const db = getCoreDb();
  const rows = await db
    .select({ jobId: finSyncJobs.id, orgId: finSyncJobs.orgId, provider: finSyncJobs.provider })
    .from(finSyncJobs)
    .where(
      or(
        eq(finSyncJobs.status, 'queued'),
