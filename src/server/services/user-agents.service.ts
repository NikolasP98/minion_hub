...
export async function listAgentAssignments(ctx: TenantContext, serverId: string) {
  return ctx.db.select().from(userAgents).where(eq(userAgents.serverId, serverId));
}
