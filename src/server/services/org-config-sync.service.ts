...
/** P0 classification primitive (channel-scoping-fix §3): channelId → accountId →
 *  owning profile id, for USER-scoped accounts only (org-scoped rows are absent
 *  here, already covered by accountOrgs). Additive sibling map — no enforcement. */
export type AccountOwnersMap = Record<string, Record<string, string>>;
...
/** Pure: PG-core channel rows → `accountOwners[type][accountId] = ownerProfileId`,
 *  USER-scoped accounts only (rows with no owner are org-scoped via accountOrgs,
 *  and are simply absent here — P0, no enforcement). */
export function buildAccountOwners(
  rows: { type: ChannelType; accountId: string | null; ownerProfileId: string | null }[],
): AccountOwnersMap {
  const acc: AccountOwnersMap = {};
  for (const r of rows) {
    const accountId = r.accountId?.trim();
    if (!accountId || !r.ownerProfileId) continue;
    (acc[r.type] ??= {})[accountId] = r.ownerProfileId;
  }
  return acc;
}
...
/** Two-level authoritative replace for a scalar leaf (accountOwners: type →
 *  accountId → profileId). Mirrors `replaceNested`'s null-out-what's-gone
 *  semantics, for a Record<string,string> leaf instead of Record<string,string[]>. */
export function replaceNestedScalar(
  current: Record<string, Record<string, unknown>>,
  next: AccountOwnersMap,
): Record<string, Record<string, string | null> | null> {
  const patch: Record<string, Record<string, string | null> | null> = {};
  for (const type of new Set([...Object.keys(current ?? {}), ...Object.keys(next)])) {
    if (!(type in next)) {
    ...
    const inner: Record<string, string | null> = { ...next[type] };
    for (const accountId of Object.keys(current?.[type] ?? {})) {
      if (!(accountId in next[type])) inner[accountId] = null; // unowned/removed
    }
    patch[type] = inner;
  }
  return patch;
...
export async function reconcileOrgConfig(gatewayId: string): Promise<{
  ...
  accountOwners: AccountOwnersMap;
  ...
  const [channelRows, disabledRows] = await Promise.all([
    db
      .select({
        ...
        // Raw column: the hub consumes @minion-stack/db as a VENDORED TARBALL
        // (package.json → file:deps/minion-stack-db-*.tgz), so the ownerProfileId
        // column added to packages/db/src/pg/schema/channels.ts is not visible to
        // the hub's Drizzle types until that dep is repacked+bumped. Selecting the
        // column directly keeps P0 correct today; swap to `channels.ownerProfileId`
        // on the next db-package release.
  ...
  const accountOwners = buildAccountOwners(channelRows as never);
  ...
  if (!orgConfigPushEnabled()) return { accountOrgs, accountOwners, orgDisabled };
  ...
  for (let attempt = 0; attempt < RECONCILE_MAX_ATTEMPTS; attempt += 1) {
    ...
    >;
    const curAccountOwners = (cur.channels?.accountOwners ?? {}) as Record<
      string,
      Record<string, unknown>
    >;
    ...
    if (isDangerousEmptyWipe(accountOrgs, orgDisabled, curAccountOrgs, curOrgDisabled)) {
      ...
      return { accountOrgs, accountOwners, orgDisabled };
    }
    ...
    try {
      await gatewayCall('config.patch', {
        raw: JSON.stringify({
          channels: {
            accountOrgs: replaceNested(curAccountOrgs, accountOrgs),
            accountOwners: replaceNestedScalar(curAccountOwners, accountOwners),
          },
...
type GatewayMaps = {
  ...
  channels?: {
    accountOrgs?: Record<string, Record<string, unknown>>;
    accountOwners?: Record<string, Record<string, unknown>>;
  };
};
