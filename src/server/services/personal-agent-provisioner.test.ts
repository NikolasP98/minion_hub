...
const mockGetPersonalAgent =
  vi.fn<(ctx: unknown, userId: string) => Promise<PersonalAgentRow | null>>();
...
describe('getPendingProvisioningForUser', () => {
  it('returns the agent row and payload if user has a pending agent', async () => {
    ...
    const result = await getPendingProvisioningForUser(
      { db: {}, tenantId: 't1' } as never,
      'user-1',
    );
