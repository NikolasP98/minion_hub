...
/** `cbc:Percent` from the SAME rate `computeTotals` divides by — declaring a
 *  different number here than what was actually divided is the "internally
 *  consistent but wrong" failure mode this parameter exists to prevent. At
 *  most 2 decimals, trailing zeros trimmed (18, not 18.00). */
function formatPercent(igvRate: number): string {
  return String(round(igvRate * 100, 2));
}

function lineXml(id: number, l: LineTotals, percentStr: string): string {
...
<cbc:TaxAmount currencyID="PEN">${l.igv.toFixed(2)}</cbc:TaxAmount>
<cac:TaxCategory>
<cbc:Percent>${percentStr}</cbc:Percent>
...
export function buildInvoiceXml(inv: EmissionInvoice): string {
  ...
  const percentStr = formatPercent(inv.igvRate);
  const lines = totals.lines.map((l, i) => lineXml(i + 1, l, percentStr)).join('\n');
