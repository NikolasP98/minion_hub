...
// resolution is deterministic per token, so memoizing (token → {tenantId,
// serverId}) collapses the repeated scan+decrypt to a Map lookup. Only
// SUCCESSFUL resolutions are cached, so a freshly-provisioned token is never
// stuck behind a cached miss; staleness (token revoked/remapped) is bounded by
// the TTL.
interface TokenCacheEntry {
  value: { tenantId: string; serverId: string };
  expires: number;
}
const SERVER_TOKEN_TTL_MS = 60_000;
...
export async function resolveIdentity(event: RequestEvent): Promise<IdentityResolution> {
  ...
  const isServerTokenPath =
    ...
    // on-the-fly CRM/finance analytics for the floating assistant; the handler
    ...
    // gateway→hub Pulse proposal-card ingest (gateway pushes explicit orgId,
    // no mailbox→org lookup needed); the handler requires locals.serverId.
    path === '/api/gateway/pulse/proposals' ||
    ...
    path.startsWith('/api/gateway/actions/') ||
    path.startsWith('/api/gateway/query/') ||
    path === '/api/gateway/pages' ||
    // AI-Brains agent surface (P4): same trusted-principal + per-brain access checks.
    path === '/api/gateway/brains' ||
    path === '/api/gateway/brain-search' ||
    path === '/api/gateway/brain-remember' ||
    // gateway→hub channel-config endpoints (gateway.json→DB migration): hydration
    // (/resolved) + all-orgs backfill (/backfill). Each handler requires
    // locals.serverId and resolves the gateway's channels. Prefix so new internal
    // channel endpoints don't each need a separate allowlist entry.
    path.startsWith('/api/internal/channels/');
