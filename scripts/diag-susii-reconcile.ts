...
async function main() {
  await withOrg(FACES_ORG, async (tx) => {
    ...
    console.log(
      `  total with derived fallback        : S/ ${a.total_with_derivation}   <-- vs SUSII 11,100`,
    );
    ...
    for (const r of miss) {
      console.log(
        `- #${r.number} ${String(r.client_name).slice(0, 26).padEnd(26)} lima=${r.lima} total=${r.total ?? 'NULL'} derived=${r.derived}`,
      );
    }
    ...
    `);
    const [c] =
      await tx`select count(*)::int n, round(coalesce(sum(total),0)::numeric,2) as t from fin_invoices`;
    console.log(
      `invoices with NULL total : ${b.docless_rows} of ${c.n} (${((b.docless_rows / c.n) * 100).toFixed(1)}%)`,
    );
    ...
    `);
    console.log(
      `checked ${d.n} doc-backed rows -> ${d.mismatches} mismatches (worst delta S/ ${d.worst})`,
    );
  });
  await client.end();
}
