...
const url = (
  ...
  readFileSync(new URL('../.env.local', import.meta.url), 'utf8').match(/^SUPABASE_DB_URL=(.*)$/m)?.[1] ??
  ''
).trim().replace(/^["']|["']$/g, '');
...
for (const mod of [stock, fin, pos]) {
  for (const v of Object.values(mod)) {
    ...
    if (!db) { console.log(`TABLE MISSING IN DB   ${name}`); bad++; continue; }
    for (const c of Object.values(getTableColumns(v))) {
      if (!db.has(c.name)) { console.log(`DRIFT  ${name}.${c.name}  — drizzle has it, DB does NOT`); bad++; }
    }
  }
}
console.log(bad === 0 ? '✅ no drift — every drizzle column exists in the DB' : `❌ ${bad} drift issue(s)`);
await sql.end();
