...
test(
  'source role sees only FACES, has no CRM writes, and bounds context in SQL',
  { skip: !url },
  async () => {
 ...
 INSERT INTO crm_contact_timeline SELECT gen_random_uuid(),'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','11111111-1111-4111-8111-111111111111','message','inbound',now()-i*interval '1 minute','message' FROM generate_series(1,30) i;`);
      ...
      await sql.begin(async (tx) => {
        ...
        const [p] = await tx`select jev_shadow_source.page(null) as rows`;
        assert.equal(p.rows.length, 1);
        assert.equal(p.rows[0].contact_id, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa');
        const [a] =
          await tx`select jev_shadow_source.evidence('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa') as value`;
        assert.equal(a.value.messages.length, 25);
        const [b] =
          await tx`select jev_shadow_source.evidence('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb') as value`;
        assert.equal(b.value, null);
        const [priv] =
          await tx`select has_table_privilege(current_user,'crm_contact_tags','INSERT') as can_write`;
        assert.equal(priv.can_write, false);
      });
      await assert.rejects(
        sql.begin(async (tx) => {
          await tx`set local role minion_jev_shadow`;
          await tx`select * from crm_contacts`;
        }),
      );
    } finally {
      await sql.end();
