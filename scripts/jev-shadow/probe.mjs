...
import { makeRequest, validateResponse } from './shadow.mjs';
const tags = JSON.parse(fs.readFileSync(new URL('./rubric.json', import.meta.url), 'utf8')).tags;
const results = [];
for (const [name, text] of [
  ['spanish', 'Quiero reservar una cita. ¿Cuál es el precio? '.repeat(35)],
  ['emoji', '🙂🧑🏽‍⚕️ '.repeat(60)],
  ['ascii', 'a1Z![]? 9_@&<>/'.repeat(110)],
]) {
  ...
  const request = makeRequest({ messages }, tags);
