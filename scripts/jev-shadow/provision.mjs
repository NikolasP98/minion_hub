...
try{
 ...
 const url=new URL(process.env.SUPABASE_DB_URL);const original=decodeURIComponent(url.username);url.username='minion_jev_shadow'+(original.includes('.')?'.'+original.split('.').slice(1).join('.'):'');url.password=password;
 await sql.begin(async tx=>{
  await tx`set local statement_timeout='10s'`;
  const orgs=await tx`select id from public.organizations where slug='faces-sculptors'`;if(orgs.length!==1)throw Error('org_not_unique');
  await tx.unsafe(`CREATE ROLE minion_jev_shadow LOGIN PASSWORD '${password}' NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION CONNECTION LIMIT 2`);
 ...
 console.log(JSON.stringify({provisioned:true,scope:'faces-sculptors',source:'read-only-functions',customer_tables_modified:false}));
}catch(e){console.error(JSON.stringify({provisioned:false,code:e.code??e.message}));process.exitCode=1;}finally{await sql.end({timeout:2});}
