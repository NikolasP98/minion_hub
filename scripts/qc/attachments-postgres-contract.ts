...
export const ATTACHMENTS_POSTGRES_FILES = [
  'src/server/services/attachments.authority.sql.integration.test.ts',
  'src/server/services/attachments.lifecycle.sql.integration.test.ts',
] as const;
...
export function assertAttachmentsPostgresReport(value: unknown) {
  ...
  if (
    ...
    report.testResults.length !== ATTACHMENTS_POSTGRES_FILES.length
  ) {
    throw new Error('Native lane must include every admitted file exactly once');
  }
  let total = 0;
  for (const file of ATTACHMENTS_POSTGRES_FILES) {
  ...
  return { files: ATTACHMENTS_POSTGRES_FILES.length, passed: total, skipped: 0 };
}
