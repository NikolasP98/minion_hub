...
describe('UI audit route inventory', () => {
  it('locks the complete endpoint ledger at 144 screens and 11 redirects', async () => {
    const inventory = await buildRouteInventory({ cleanBaseline: true });

    expect(inventory.summary).toMatchObject({ endpoints: 155, screens: 144, redirects: 11 });
    expect(new Set(inventory.routes.map((route) => route.pattern)).size).toBe(155);
    expect(
